...
{
  DEBUG_API_LOG("\nEmwApiEmw::EmwApiEmw()>\n")
  DEBUG_API_LOG("EmwApiEmw::EmwApiEmw()<\n\n")
}
...
int32_t EmwApiEmw::socketClose(int32_t socketFd)
{
  ...
  if ((0 <= socketFd)) {
    ...
    if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
        reinterpret_cast<uint8_t *>(&command_data.closeParams), (uint16_t)sizeof(command_data.closeParams),
...
int32_t EmwApiEmw::socketCreate(int32_t domain, int32_t type, int32_t protocol)
{
  ...
  if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
      reinterpret_cast<uint8_t * >(&command_data.createParams), (uint16_t)sizeof(command_data.createParams),
      reinterpret_cast<uint8_t * >(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
                                 ...
                                 const EmwAddress::SockAddr_t *socketAddressPtr, int32_t socketAddressLength)
{
  ...
  if ((0 <= socketFd) && (nullptr != socketAddressPtr) && (0 < socketAddressLength)) {
    ...
    if (is_to_do_ipc_request) {
      ...
      command_data.connectParams.length = (EmwAddress::SockLen_t)socketAddressLength;

      if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
          reinterpret_cast<uint8_t * >(&command_data.connectParams), (uint16_t)sizeof(command_data.connectParams),
          reinterpret_cast<uint8_t * >(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
        if ((int32_t)EmwCoreIpc::eSUCCESS == response_buffer.status) {
          status = 0;
        }
...
int32_t EmwApiEmw::socketGetAddrInfo(const char *nodeNameStringPtr, const char *serviceNameStringPtr,
                                     const EmwAddress::AddrInfo_t *hintsPtr, EmwAddress::AddrInfo_t *resultPtr)
{
  int32_t ret = -4;
  DEBUG_API_LOG("\nEmwApiEmw::socketGetAddrInfo()>\n")

  if ((nullptr != nodeNameStringPtr) && (nullptr != hintsPtr) && (nullptr != resultPtr)) {
    ...
    command_data.getAddrInfoParams.hints = *hintsPtr;
    if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
        reinterpret_cast<uint8_t * >(&command_data.getAddrInfoParams), (uint16_t)sizeof(command_data.getAddrInfoParams),
        reinterpret_cast<uint8_t * >(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
      if ((int32_t)EmwCoreIpc::eSUCCESS == response_buffer.status) {
        ...
        resultPtr->canonName[(int32_t)sizeof(resultPtr->canonName) - 1] = '\0';
        ret = 0;
      }
      else {
...
int32_t EmwApiEmw::socketGetHostByName(EmwAddress::SockAddr_t *socketAddressPtr, const char *nameStringPtr)
{
  ...
  if ((nullptr != socketAddressPtr) && (nullptr != nameStringPtr)) {
    if (strlen(nameStringPtr) < sizeof(command_data.getHostByNameParams.name)) {
      ...
      if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
          reinterpret_cast<uint8_t * >(&command_data.getHostByNameParams),
          (uint16_t)sizeof(command_data.getHostByNameParams),
          reinterpret_cast<uint8_t * >(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
        if ((int32_t)EmwCoreIpc::eSUCCESS == response_buffer.status) {
          /* Only for IPv4 address. */
          EmwAddress::SockAddrIn_t *const socket_address_in_ptr \
            = reinterpret_cast<EmwAddress::SockAddrIn_t *>(socketAddressPtr);
          socket_address_in_ptr->length = (uint8_t)sizeof(*socket_address_in_ptr);
                                    ...
                                    int32_t optionName, void *optionValuePtr, uint32_t *optionLengthPtr)
{
  ...
  if ((0 <= socketFd) && (nullptr != optionValuePtr) && (nullptr != optionLengthPtr)) {
    ...
    if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
        reinterpret_cast<uint8_t *>(&command_data.getSockOptParams), (uint16_t)sizeof(command_data.getSockOptParams),
        reinterpret_cast<uint8_t *>(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
...
int32_t EmwApiEmw::socketSend(int32_t socketFd, const uint8_t *dataPtr, int32_t dataLength, int32_t flags)
{
  ...
  if ((0 <= socketFd) && (nullptr != dataPtr) && (0 < dataLength)) {
    ...
    if ((data_len + sizeof(EmwCoreIpc::IpcSocketSendParams_t) - 1U) > EmwNetworkStack::NETWORK_IPC_PAYLOAD_SIZE) {
      data_len = EmwNetworkStack::NETWORK_IPC_PAYLOAD_SIZE - (sizeof(EmwCoreIpc::IpcSocketSendParams_t) - 1U);
    }
    {
      const uint16_t command_data_size = (uint16_t)(sizeof(EmwCoreIpc::IpcSocketSendParams_t) - 1U + data_len);
      EmwCoreIpc::IpcSocketSendParams_t *const command_data_ptr \
        = static_cast<EmwCoreIpc::IpcSocketSendParams_t *> \
          (EmwOsInterface::malloc(sizeof(EmwCoreIpc::CmdParams_t) + (size_t)command_data_size));

      if (nullptr != command_data_ptr) {
        ...
        (void) memcpy(&command_data_ptr->sendParams.buffer[0], dataPtr, data_len);
        command_data_ptr->sendParams.size = data_len;
        ...
        EmwOsInterface::free(command_data_ptr);
      }
    }
  }
                                    ...
                                    int32_t optionName, const void *optionValuePtr, int32_t optionLength)
{
  ...
  if ((0 <= socketFd) && (nullptr != optionValuePtr) && (0 < optionLength)) {
    ...
    command_data.setSockOptParams.length =
      (optionLength > (int32_t)sizeof(command_data.setSockOptParams.value)) \
      ? (EmwAddress::SockLen_t)sizeof(command_data.setSockOptParams.value) \
      : (EmwAddress::SockLen_t)optionLength;
    (void)memcpy(&command_data.setSockOptParams.value[0], optionValuePtr,
                 command_data.setSockOptParams.length);

    if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
        reinterpret_cast<uint8_t * >(&command_data.setSockOptParams), (uint16_t)sizeof(command_data.setSockOptParams),
        reinterpret_cast<uint8_t * >(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
      if ((int32_t)EmwCoreIpc::eSUCCESS == response_buffer.status) {
        status = 0;
      }
...
int32_t EmwApiEmw::socketShutDown(int32_t socketFd, int32_t mode)
{
  ...
  if (0 <= socketFd) {
    ...
    if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
        reinterpret_cast<uint8_t *>(&command_data.shutDownParams), (uint16_t)sizeof(command_data.shutDownParams),
        reinterpret_cast<uint8_t *>(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
      if ((int32_t)EmwCoreIpc::eSUCCESS == response_buffer.status) {
        status = 0;
      }
...
int32_t EmwApiEmw::socketReceive(int32_t socketFd, uint8_t *bufferPtr, int32_t bufferLength, int32_t flags)
{
  ...
  if ((0 <= socketFd) && (nullptr != bufferPtr) && (0 < bufferLength)) {
    EmwCoreIpc::SocketReceiveResponseParams_t *response_buffer_ptr;
    size_t data_len = (size_t)bufferLength;
    uint16_t response_buffer_size;

    status = -1;

    if ((data_len + sizeof(EmwCoreIpc::SocketReceiveResponseParams_t) - 1U) > EmwNetworkStack::NETWORK_IPC_PAYLOAD_SIZE) {
      data_len = EmwNetworkStack::NETWORK_IPC_PAYLOAD_SIZE - (sizeof(EmwCoreIpc::SocketReceiveResponseParams_t) - 1U);
    }

    response_buffer_size = (uint16_t)(sizeof(EmwCoreIpc::SocketReceiveResponseParams_t) - 1U + data_len);
    response_buffer_ptr \
      = static_cast<EmwCoreIpc::SocketReceiveResponseParams_t *>(EmwOsInterface::malloc((size_t)response_buffer_size));

    if (nullptr != response_buffer_ptr) {
      EmwCoreIpc::IpcSocketReceiveParams_t command_data(socketFd, data_len, flags);

      response_buffer_ptr->received = 0;

      if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
          reinterpret_cast<uint8_t * >(&command_data.receiveParams), (uint16_t)sizeof(command_data.receiveParams),
          reinterpret_cast<uint8_t * >(response_buffer_ptr), &response_buffer_size, EMW_CMD_TIMEOUT)) {
        if (response_buffer_ptr->received > 0) {
          const size_t received_len = (size_t)response_buffer_ptr->received;
      ...
      EmwOsInterface::free(response_buffer_ptr);
    }
  }
                                ...
                                int32_t count, int32_t delayInMs, int32_t responses[])
{
  ...
  if ((nullptr != hostnameStringPtr) && (0 < count)) {
    ...
    uint16_t response_buffer_size = (uint16_t)sizeof(EmwCoreIpc::WiFiPingResponseParams_t);
    const int32_t count_max = (int32_t)(sizeof(response_buffer.delaysInMs) / sizeof(response_buffer.delaysInMs[0]));
    ...
    if (EmwCoreIpc::eSUCCESS == EmwCoreIpc::request(*this,
        reinterpret_cast<uint8_t *>(&command_data.pingParams), (uint16_t)sizeof(command_data.pingParams),
        reinterpret_cast<uint8_t *>(&response_buffer), &response_buffer_size, EMW_CMD_TIMEOUT)) {
...
EmwAddress::SockAddrIn_t EmwApiEmw::socketAddressIn_FromPacked(const EmwAddress::SockAddrStorage_t *socketAddressPtr)
{
  ...
  {
    uint16_t port_in = (uint16_t)socketAddressPtr->data1[0];
    port_in |= (uint16_t)((uint16_t)socketAddressPtr->data1[1] << 8);
...
EmwAddress::SockAddrIn6_t EmwApiEmw::socketAddressIn6_FromPacked(const EmwAddress::SockAddrStorage_t
    *socketAddressPtr)
{
...
EmwAddress::SockAddrStorage_t EmwApiEmw::socketAddressIn_ToPacked(const EmwAddress::SockAddr_t *socketAddressPtr)
{
  ...
  s_address_storage.data1[0] = (uint8_t)(s_address_in_ptr->port);
  s_address_storage.data1[1] = (uint8_t)(s_address_in_ptr->port >> 8);
...
EmwAddress::SockAddrStorage_t EmwApiEmw::socketAddressIn6_ToPacked(const EmwAddress::SockAddr_t *socketAddressPtr)
{
  ...
  s_address_storage.data1[0] = (uint8_t)(s_address_in6_ptr->port);
  s_address_storage.data1[1] = (uint8_t)(s_address_in6_ptr->port >> 8);
