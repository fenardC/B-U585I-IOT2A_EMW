...
{
  DEBUG_STD_PRINTF(" AppConsoleIperf::~AppConsoleIperf()> %p\n", static_cast<const void*>(&this->lwipNetif))
  lwiperf_abort(this->connectionPtr);
  DEBUG_STD_PRINTF(" AppConsoleIperf::~AppConsoleIperf()<\n")
}
...
std::int32_t AppConsoleIperf::execute(std::int32_t argc, const char *argvPtrs[]) noexcept
{
  ...
  if (this->connectionPtr == nullptr) {
    ...
    if ((argc == 3) && (cmd_argv == "-c")) {
      ...
      if (ipaddr_aton(remote_ip_argv.data(), &remote_ip_addr) == 1) {
        this->connectionPtr = lwiperf_start_tcp_client_default(&remote_ip_addr, &AppConsoleIperf::ReportFunction, this);
        if (this->connectionPtr != nullptr) {
          status = 0;
      ...
      this->connectionPtr = lwiperf_start_tcp_server_default(&AppConsoleIperf::ReportFunction, this);
                                     ...
                                     u32_t bytesTransferred, u32_t durationInMs, u32_t bandwidthKBitPerSec) noexcept
{
  ...
  console_ptr->connectionPtr = nullptr;
}
