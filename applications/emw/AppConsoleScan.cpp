...
int32_t AppConsoleScan::execute(int32_t argc, char *argvPtrs[])
{
  ...
  if (EmwApiBase::eEMW_STATUS_OK == emw_ptr->scan(EmwApiBase::ePASSIVE, nullptr, 0)) {
    status = 0;
...
#define AP_SSID_LENGTH sizeof((static_cast<EmwApiBase::ApInfo_t *>(0))->ssid)
      ...
      std::printf("%2" PRIi32 "\t%32s ch %2" PRIi32 " rss %" PRIi32 " Security %10s"
                  ...
                  AppConsoleScan::securityToString(emw_security[APs[i].security]),
    ...
    std::printf("%s: operation failed (%" PRIi32 ")!\n", this->getName(), static_cast<int32_t>(status));
  }
  STD_PRINTF("\nAppConsoleScan::execute()<\n\n")
  return status;
}
