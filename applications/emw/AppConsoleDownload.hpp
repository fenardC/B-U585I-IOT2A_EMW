...
class AppConsoleDownload final : public Cmd {
  public:
    explicit AppConsoleDownload(EmwApiEmw &wifiEmw) noexcept;
  public:
  ...
  private:
    std::int32_t doDownload(const char *hostPtr, const char *requestPtr) noexcept;
  private:
    std::int32_t doDownload6(const char *hostPtr, const char *requestPtr) noexcept;
  private:
    std::int32_t doDownloadFile(const char *hostPtr, const char *requestPtr) const noexcept;
  private:
    std::int32_t readResponse(std::uint8_t *headerPtr, std::uint32_t maximumSize,
                              std::uint32_t &retSize) const noexcept;
  private:
    std::int32_t testResponse(AppConsoleDownload::HttpContext_t &context, std::uint8_t *bufferPtr) const noexcept;
