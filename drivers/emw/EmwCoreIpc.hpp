...
#define CMD_BASE ((uint16_t)(ID_NONE))
  ...
  public:
    static Status testIpcEcho(const class EmwApiCore &core, uint8_t *dataInPtr, uint16_t dataInLength,
                              uint8_t *dataOutPtr, uint16_t *dataOutLengthPtr, uint32_t timeoutMs);
  public:
    static Status unInitialize(void);
  public:
    typedef __PACKED_STRUCT CmdParams_s {
      CmdParams_s(void): reqId{0U, 0U, 0U, 0U}, apiId{0U, 0U} {}
      explicit CmdParams_s(uint16_t apiId)
        : reqId{0U, 0U, 0U, 0U}
        , apiId{(uint8_t)(apiId & 0x00FFU), (uint8_t)((apiId & 0xFF00U) >> 8)} {}
      uint8_t reqId[4];
      uint8_t apiId[2];
    } CmdParams_t;
    ...
    typedef __PACKED_STRUCT IpcNoParams_s {
      IpcNoParams_s(void): ipcParams(), dataNull(0U) {}
      explicit IpcNoParams_s(EmwCoreIpc::ApiId apiId): ipcParams(apiId), dataNull(0U) {}
      CmdParams_t ipcParams;
      uint8_t dataNull;
    } IpcNoParams_t;
