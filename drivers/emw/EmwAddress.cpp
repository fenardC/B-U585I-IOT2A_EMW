...
int32_t EmwAddress::asciiToNetwork(const char *addressStringPtr, EmwAddress::IpAddr_t &address)
{
  ...
  byte_in_string = static_cast<uint8_t>(*char_ptr);
  for (;;) {
    /**
    ...
    if (!EmwAddress::isDigit(byte_in_string)) {
      ...
      for (;;) {
        if (EmwAddress::isDigit(byte_in_string)) {
          ...
          val = (val << 4) \
                | ((byte_in_string + 10U \
                    - ((EmwAddress::isLower(byte_in_string)) ? 'a' : 'A')));
          ++char_ptr;
          byte_in_string = static_cast<uint8_t>(*char_ptr);
        }
        else {
  ...
  if ((byte_in_string != '\0') && (EmwAddress::isSpace((byte_in_string)) == false)) {
    status = 0;
  }
  else {
  ...
  for (auto char_ptr = addressStringPtr; *char_ptr != 0; char_ptr++) {
    if (*char_ptr == ':') {
      ...
      current_block_value = (current_block_value << 4) +
                            (EmwAddress::isDigit(*char_ptr) ? (*char_ptr - '0') :
                             (10 + (EmwAddress::isLower(*char_ptr) ? *char_ptr - 'a' : *char_ptr - 'A')));
    }
    else {
...
char *EmwAddress::networkToAscii(const EmwAddress::IpAddr_t &address, char buffer[], const std::size_t bufferSize)
{
  uint32_t length = 0U;
  const uint8_t ip_addr_bytes[4] = {
    static_cast<uint8_t>(address.addr & 0x000000FFU),
    static_cast<uint8_t>((address.addr & 0x0000FF00U) >> 8),
    static_cast<uint8_t>((address.addr & 0x00FF0000U) >> 16),
    static_cast<uint8_t>((address.addr & 0xFF000000U) >> 24)
  };
...
char *EmwAddress::networkToAscii(const EmwAddress::Ip6Addr_t &address, char buffer[], const std::size_t bufferSize)
{
  uint32_t current_block_index;
...
{
  return (((static_cast<uint32_t>(hostLong) & 0xff000000U) >> 24) | \
          ((static_cast<uint32_t>(hostLong) & 0x00ff0000U) >> 8) | \
          ((static_cast<uint32_t>(hostLong) & 0x0000ff00U) << 8) | \
          ((static_cast<uint32_t>(hostLong) & 0x000000ffU) << 24));
}
