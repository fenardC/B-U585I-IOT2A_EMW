...
EmwOsInterface::Status EmwOsInterface::addSemaphoreHook(semaphore_t *semaphorePtr, runner_hook_t waiter, void *THIS,
...
EmwOsInterface::Status EmwOsInterface::createMutex(mutex_t *mutexPtr, const char *mutexNamePtr)
{
  ...
  if (nullptr != mutexPtr) {
    ...
    status = EmwOsInterface::createSemaphore(reinterpret_cast<semaphore_t *>(mutexPtr), mutexNamePtr, 1, 1);
#else
...
EmwOsInterface::Status EmwOsInterface::takeMutex(mutex_t *mutexPtr, uint32_t timeoutInMs)
{
  ...
  if (nullptr != mutexPtr) {
#if defined(IMPLEMENT_MUTEX_AS_SEMAPHORE)
    status = EmwOsInterface::takeSemaphore(reinterpret_cast<semaphore_t *>(mutexPtr), timeoutInMs);
#else
    (void) timeoutInMs;
    status = EmwOsInterface::eOK;
#endif /* IMPLEMENT_MUTEX_AS_SEMAPHORE */
...
EmwOsInterface::Status EmwOsInterface::releaseMutex(mutex_t *mutexPtr)
{
  ...
  if (nullptr != mutexPtr) {
#if defined(IMPLEMENT_MUTEX_AS_SEMAPHORE)
    status = EmwOsInterface::releaseSemaphore(reinterpret_cast<semaphore_t *>(mutexPtr));
#else
    status = EmwOsInterface::eOK;
#endif /* IMPLEMENT_MUTEX_AS_SEMAPHORE */
  }
...
EmwOsInterface::Status EmwOsInterface::deleteMutex(mutex_t *mutexPtr)
{
  ...
  if (nullptr != mutexPtr) {
#if defined(IMPLEMENT_MUTEX_AS_SEMAPHORE)
    status = EmwOsInterface::deleteSemaphore(reinterpret_cast<semaphore_t *>(mutexPtr));
#else
...
EmwOsInterface::Status EmwOsInterface::addMessageQueueHook(queue_t *queuePtr, runner_hook_t waiter, void *THIS,
    const class EmwApiCore *corePtr)
{
  EmwOsInterface::Status status = EmwOsInterface::eERROR;
  if (nullptr != queuePtr) {
