...
namespace EmwApiBase {
  ...
  typedef __PACKED_STRUCT IpAttributes_s {
    constexpr IpAttributes_s(void): ipAddressLocal{0}, networkMask{0}, gatewayAddress{0}, dnsServerAddress{0} {}
  ...
  typedef uint8_t Security_t;

  typedef __PACKED_STRUCT ConnectAttributes_s {
    constexpr ConnectAttributes_s(void): bssid{0}, channel(0U), security(0U) {}
    uint8_t bssid[6];
    uint8_t channel;
  ...
  typedef __PACKED_STRUCT EapAttributes_s {
    constexpr EapAttributes_s(void): eapType(0U), rootCaPtr(nullptr), clientCertificatePtr(nullptr), clientKeyPtr(nullptr) {}
    uint8_t eapType;
  ...
  typedef __PACKED_STRUCT ApInfo_s{
    constexpr ApInfo_s(void): rssi(0), ssid{0}, bssid{0}, channel(0), security(0U) {}
    int32_t rssi;
    char ssid[32U + 1U];
    uint8_t bssid[6];
    int32_t channel;
  ...
  typedef __PACKED_STRUCT ScanResults_s{
    constexpr ScanResults_s(void): count(0U), accessPoints{} {}
    uint8_t count;
    EmwApiBase::ApInfo_t accessPoints[10U];
  } ScanResults_t;

  typedef struct SoftApSettings_s {
    constexpr SoftApSettings_s(void): ssidString{0}, passwordString{0}, channel(0U), ip() {}
  ...
  typedef void (*FotaStatusCallback_t)(EmwApiBase::FotaStatus status, uint32_t arg);
  typedef void (*NetlinkInputCallback_t)(void *networkBufferPtr, uint32_t arg);

  typedef __PACKED_STRUCT LinkInformation_s{
    constexpr LinkInformation_s(void): isConnected(0), ssid{0}, bssid{0}, security(0), channel(0), rssi(0) {}
    uint8_t isConnected;
    char ssid[32];
    uint8_t bssid[6];
    uint8_t security;
