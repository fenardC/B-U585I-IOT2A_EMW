...
static EXTI_HandleTypeDef *getExtiFlow(void)
{
...
static EXTI_HandleTypeDef *getExtiNotify(void)
{
  static EXTI_HandleTypeDef external_interrupt =
...
int32_t EmwIoHardware::initialize(EmwIoSpi::Stm32Hw_s &configuration)
{
  int32_t status;
  ...
  configuration.resetPin = GPIO_PIN_15;
  status = 0;
#else
  status = -1;
#endif /* EMW_USE_SPI || EMW_USE_SPI_DMA */
  return status;
}
