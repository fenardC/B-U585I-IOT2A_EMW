...
#define VOID_MEMSET_ARRAY(DEST, VAL) static_cast<void>(std::memset(static_cast<void *>(DEST), (VAL), sizeof((DEST))))

EMW_STATS_DECLARE()

const char *EmwApiCore::productName = "MXCHIP-WIFI";
const char *EmwApiCore::productIdentifier = "EMW3080B";

static EmwOsInterface::Mutex_t DeviceAvailableLock;

static std::uint8_t ToIpcInterface(EmwApiBase::EmwInterface interface);

const char *EmwApiCore::getConfigurationString(void) noexcept
{
  static const char configuration_string[] = {EMW_IO_NAME_STRING ", " RTOS_NAME_STRING ", " NETWORK_NAME_STRING};
  return configuration_string;
}

void EmwApiCore::getStatistics(void) noexcept
{
  EMW_STATS_LOG()
}
                                       ...
                                       const EmwApiBase::IpAttributes_t &ipAttributes) noexcept
{
  ...
  if ((ssid_string_length > ssid_length_max) || (password_string_length > password_length_max)) {
    ...
    if (attributes.bssid[0] != 0U) {
      command_data.connectParams.useAttribute = 1U;
      static_cast<void>(std::memcpy(&command_data.connectParams.attr.bssid, &attributes.bssid,
    ...
    if (ipAttributes.ipAddressLocal[0] != '\0') {
      command_data.connectParams.useIp = 1U;
      STRING_COPY_TO_ARRAY_CHAR(command_data.connectParams.ip.ipAddressLocal, ipAttributes.ipAddressLocal);
      STRING_COPY_TO_ARRAY_CHAR(command_data.connectParams.ip.networkMask, ipAttributes.networkMask);
      STRING_COPY_TO_ARRAY_CHAR(command_data.connectParams.ip.gatewayAddress, ipAttributes.gatewayAddress);
    ...
    const EmwApiBase::IpAttributes_t &ipAttributes) noexcept
{
  ...
  if ((ssid_string_length > ssid_length_max) || (password_string_length > password_length_max)) {
    ...
    if (attributes.bssid[0] != 0U) {
      command_data.connectParams.useAttribute = 1U;
      static_cast<void>(memcpy(&command_data.connectParams.attr.bssid, &attributes.bssid,
                               sizeof(command_data.connectParams.attr.bssid)));
      command_data.connectParams.attr.channel = attributes.channel;
      command_data.connectParams.attr.security = attributes.security;
    }
    ...
    const EmwApiBase::IpAttributes_t &ipAttributes) noexcept
{
  ...
  if ((ssid_string_length > ssid_length_max) || (identity_string_length > identity_length_max) \
      || (password_string_length > password_length_max)) {
    status = EmwApiBase::eEMW_STATUS_PARAM_ERROR;
  }
  else {
    ...
    if (eapAttributes.eapType != 0U) {
      if ((eapAttributes.eapType != EmwApiBase::eEAP_TYPE_TLS) \
          && (eapAttributes.eapType != EmwApiBase::eEAP_TYPE_TTLS) \
          && (eapAttributes.eapType != EmwApiBase::eEAP_TYPE_PEAP)) {
        status = EmwApiBase::eEMW_STATUS_PARAM_ERROR;
      }
      else {
        command_data.eapConnectParams.attr.eapType = eapAttributes.eapType;
        if (eapAttributes.rootCaPtr != nullptr) {
          status = this->setEapCert(EmwCoreIpc::EAP_ROOTCA,
                                    eapAttributes.rootCaPtr, std::strlen(eapAttributes.rootCaPtr));

          if (status == EmwApiBase::eEMW_STATUS_OK) {
            command_data.eapConnectParams.attrUsed = 1U;
          }
        }
        if ((status == EmwApiBase::eEMW_STATUS_OK) && (eapAttributes.clientCertificatePtr != nullptr)) {
        ...
        if ((status == EmwApiBase::eEMW_STATUS_OK) && (eapAttributes.clientKeyPtr != nullptr)) {
    ...
    EmwApiBase::EmwInterface interface) noexcept
{
  ...
  if (this->::EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
                                  BYTES_REF_CAST(&response_buffer), response_buffer_size,
                                  EmwApiCore::GET_IP_ADDRESS_TIMEOUT) == EmwCoreIpc::eSUCCESS) {
    if (response_buffer.status == 0) {
      {
        EmwAddress::IpAddr_t ip;
        EmwAddress::AsciiToNetwork(&response_buffer.ip.ipAddressLocal[0], ip);
        static_cast<void>(std::memcpy(&this->stationSettings.ipAddress[0], &ip.addr, sizeof(this->stationSettings.ipAddress)));
      }
      {
        EmwAddress::IpAddr_t netmask;
        EmwAddress::AsciiToNetwork(&response_buffer.ip.networkMask[0], netmask);
        static_cast<void>(std::memcpy(&this->stationSettings.ipMask[0], &netmask.addr, sizeof(this->stationSettings.ipMask)));
      }
      {
        EmwAddress::IpAddr_t gw;
        EmwAddress::AsciiToNetwork(&response_buffer.ip.gatewayAddress[0], gw);
        static_cast<void>(std::memcpy(&this->stationSettings.gatewayAddress[0], &gw.addr,
      ...
      }
      {
        ...
        static_cast<void>(std::memcpy(&this->stationSettings.dns1[0], &dns.addr, sizeof(this->stationSettings.dns1)));
      }
      static_cast<void>(std::memcpy(ipAddressBytes, this->stationSettings.ipAddress,
  ...
  if (addressSlot < 3U) {
    ...
    if (this->EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
      ...
      if (response_buffer.status == 0) {
        ...
        static_cast<void>(std::memcpy(ip6AddressBytes, response_buffer.ip6, sizeof(response_buffer.ip6)));
        status = EmwApiBase::eEMW_STATUS_OK;
      }
    }
...
EmwApiBase::Status EmwApiCore::getStationMacAddress(EmwApiCore::MacAddress_t &mac) const noexcept
{
  ...
  if (EmwApiCore::interfaces > 0U) {
    static_cast<void>(std::memcpy(mac.bytes, &this->systemInformations.mac48bitsStation[0], sizeof(mac.bytes)));
    status = EmwApiBase::eEMW_STATUS_OK;
  }
...
EmwApiBase::Status EmwApiCore::getSoftApMacAddress(EmwApiCore::MacAddress_t &mac) noexcept
{
  ...
  if (this->::EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
                                  ...
                                  EMW_CMD_TIMEOUT) == EmwCoreIpc::eSUCCESS) {
...
EmwApiBase::Status EmwApiCore::getVersion(char (&version)[25], std::uint32_t versionSize) noexcept
{
  ...
  if ((versionSize > 0U) && (versionSize <= 25U)) {
    ...
    if (this->::EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
                                    BYTES_REF_CAST(&firmware_revision[0]), response_buffer_size,
                                    EmwCoreIpc::EMW_CMD_TIMEOUT) == EmwCoreIpc::eSUCCESS) {
...
int8_t EmwApiCore::getScanResults(std::uint8_t (&results)[480], std::uint8_t number) const noexcept
{
  ...
  if (number != 0U) {
    copy_number = (this->lastScanResults.count < number) ? this->lastScanResults.count : number;
    static_cast<void>(std::memcpy(results, this->lastScanResults.accessPoints,
                                  copy_number * sizeof(this->lastScanResults.accessPoints[0])));
  }
...
EmwApiBase::Status EmwApiCore::initialize(void) noexcept
{
...

  {
    ...
    if (EmwApiCore::interfaces == 0U) {
...
#if defined(EMW_WITH_RTOS)
      {
        ...
        EmwOsInterface::AssertAlways(os_status == EmwOsInterface::eOK);
      }
      EmwOsInterface::DelayTicks(1U);
#endif /* EMW_WITH_RTOS */
      EmwApiCore::interfaces = 1U;
    }
    else {
      EmwApiCore::interfaces = EmwApiCore::interfaces + 1;
      status = EmwApiBase::eEMW_STATUS_OK;
    }
  }
  {
    ...
    if (this->::EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
                                    BYTES_REF_CAST(&this->systemInformations.firmwareRevision[0]), response_buffer_size,
                                    EmwCoreIpc::EMW_CMD_TIMEOUT) == EmwCoreIpc::eSUCCESS) {
      {
        const char firmware_revision_required_string[] = "V2.3.4";
        const bool out_of_date = (std::strncmp(firmware_revision_required_string,
                                               static_cast<const char *>(this->systemInformations.firmwareRevision),
                                               sizeof(firmware_revision_required_string)) > 0);
        if (out_of_date) {
  ...
  }
  {
    ...
    if (this->::EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
                                    BYTES_REF_CAST(&this->systemInformations.mac48bitsStation[0]), response_buffer_size,
                                    EmwCoreIpc::EMW_CMD_TIMEOUT) == EmwCoreIpc::eSUCCESS) {
      status = EmwApiBase::eEMW_STATUS_OK;
    }
    else {
...
std::int8_t EmwApiCore::isConnected(void) noexcept
{
  ...
  if (this->::EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
    ...
    if (response_buffer.status == 0) {
      ...
      if (this->stationSettings.isConnected > 0) {
        state = 1;
      }
    }
  }
    ...
    void *argPtr, EmwApiBase::EmwInterface interface) noexcept
{
  ...
  if (EmwApiCore::interfaces > 0U) {
    ...
    this->callbacks.wiFiStatusCallbacks[interface_index] = statusCallbackFunctionPtr;
    this->callbacks.wiFiStatusCallbackArgPtrs[interface_index] = argPtr;
    status = EmwApiBase::eEMW_STATUS_OK;
  }
                                    ...
                                    const char (&ssidString)[33], std::int32_t ssidStringLength) noexcept
{
  ...
  this->lastScanResults.count = 0U;
  VOID_MEMSET_ARRAY(&this->lastScanResults.accessPoints, 0);
  if (((scanMode == EmwApiBase::eSCAN_ACTIVE) && ((ssidStringLength <= 0) || (ssidStringLength > 32)))) {
    status = EmwApiBase::eEMW_STATUS_PARAM_ERROR;
  }
  else {
    EmwCoreIpc::IpcWiFiScanParams_t command_data;
    std::uint16_t scan_results_size = sizeof(this->lastScanResults);

    if (scanMode == EmwApiBase::eSCAN_ACTIVE) {
    ...
    if (this->::EmwCoreIpc::request(BYTES_REF_CAST(&command_data), sizeof(command_data),
                                    BYTES_REF_CAST(&this->lastScanResults), scan_results_size,
                                    EmwApiCore::SCAN_TIMEOUT_IN_MS) == EmwCoreIpc::eSUCCESS) {
      status = EmwApiBase::eEMW_STATUS_OK;
    }
  }
...
EmwApiBase::Status EmwApiCore::startSoftAp(const EmwApiBase::SoftApSettings_t &accessPointSettings) noexcept
{
  ...
  static_cast<void>(std::memcpy(command_data.softApStartParams.ssid, accessPointSettings.ssidString,
                                sizeof(command_data.softApStartParams.ssid)));
  static_cast<void>(std::memcpy(command_data.softApStartParams.key, accessPointSettings.passwordString,
...
EmwApiBase::Status EmwApiCore::unRegisterStatusCallback(EmwApiBase::EmwInterface interface) noexcept
{
  ...
  if (EmwApiCore::interfaces > 0U) {
    const std::uint8_t interface_index = (EmwApiBase::eSTATION == interface) \
                                         ? static_cast<std::uint8_t>(EmwApiBase::eWIFI_INTERFACE_STATION_IDX) \
                                         : static_cast<std::uint8_t>(EmwApiBase::eWIFI_INTERFACE_SOFTAP_IDX);
    this->callbacks.wiFiStatusCallbacks[interface_index] = nullptr;
    this->callbacks.wiFiStatusCallbackArgPtrs[interface_index] = nullptr;
    status = EmwApiBase::eEMW_STATUS_OK;
  }
    ...
    std::uint32_t length) noexcept
{
  ...
  if (certificateStringPtr != nullptr) {
    ...
    std::uint8_t (&dataOut)[], std::uint16_t &dataOutLength) noexcept
{
  ...
  if (this->::EmwCoreIpc::testEcho(dataIn, dataInLength, dataOut, dataOutLength) == EmwCoreIpc::eSUCCESS) {
    status = EmwApiBase::eEMW_STATUS_OK;
  }
  else {
    status = EmwApiBase::eEMW_STATUS_ERROR;
...
void EmwApiCore::unInitialize(void) noexcept
{
  ...
  if (EmwApiCore::interfaces == 1U) {
    EmwApiCore::interfaces = 0U;
#if defined(EMW_WITH_RTOS)
...
#endif /* EMW_API_DEBUG */
  DEBUG_API_LOG("\n[%" PRIu32 "] EmwApiCore::ReceiveThreadFunction(%p)>\n", HAL_GetTick(),
                static_cast<const void *>(argument))
...
void EmwApiCore::processEvent(EmwNetworkStack::Buffer_t *networkBufferPtr, std::uint16_t apiId) noexcept
{
  ...
  static constexpr struct Entry_s EVENTS_TABLE[] = {
    ...
    {EmwCoreIpc::eWIFI_STATUS_EVENT, EmwApiCore::ProcessWiFiStatusEvent},
...
void EmwApiCore::ProcessRebootEvent(const EmwApiCore *THIS, EmwNetworkStack::Buffer_t *networkBufferPtr) noexcept
{
  ...
  DEBUG_API_LOG("\n     EmwApiCore::ProcessRebootEvent(): EVENT: reboot done.\n")
}

void EmwApiCore::ProcessFotaStatusEvent(const EmwApiCore *THIS, EmwNetworkStack::Buffer_t *networkBufferPtr) noexcept
{
  std::uint8_t *const buffer_payload_ptr = EmwNetworkStack::GetBufferPayload(networkBufferPtr);

  const EmwApiBase::FotaStatus status \
    = *(reinterpret_cast<EmwApiBase::FotaStatus *>(EmwCoreIpc::SkipHeader(buffer_payload_ptr)));

  DEBUG_API_LOG("\n     EmwApiCore::ProcessFotaStatusEvent(): EVENT: FOTA status: %02x\n", status)
  ...
  {
    EmwApiBase::FotaStatusCallback_t const status_callback_function = THIS->callbacks.fotaStatusCallback;
    const std::uint32_t callback_arg = THIS->callbacks.fotaStatusCallbackArg;

    if (status_callback_function != nullptr) {
      status_callback_function(status, callback_arg);
    }
  }
}

void EmwApiCore::ProcessWiFiStatusEvent(const EmwApiCore *THIS, EmwNetworkStack::Buffer_t *networkBufferPtr) noexcept
{
  EmwApiBase::EmwInterface wifi_interface;
  EmwApiBase::WiFiStatusCallback_t status_callback_function = nullptr;
  void *callback_arg = nullptr;
  std::uint8_t (&buffer_payload)[] \
    = reinterpret_cast<std::uint8_t (&)[]>(* EmwNetworkStack::GetBufferPayload(networkBufferPtr));
  const EmwApiBase::WiFiStatus_t status \
    = *(static_cast<EmwApiBase::WiFiStatus_t *>(EmwCoreIpc::SkipHeader(buffer_payload)));
  const enum EmwApiBase::WiFiEvent event = static_cast<enum EmwApiBase::WiFiEvent>(status);

  DEBUG_API_LOG("\n     EmwApiCore::ProcessWiFiStatusEvent(%p): EVENT: Wi-Fi status: %02x\n",
  ...
  switch (event) {
    ...
    case EmwApiBase::eWIFI_EVENT_STA_GOT_IP: {
        ...
        status_callback_function = THIS->callbacks.wiFiStatusCallbacks[EmwApiBase::eWIFI_INTERFACE_STATION_IDX];
        callback_arg = THIS->callbacks.wiFiStatusCallbackArgPtrs[EmwApiBase::eWIFI_INTERFACE_STATION_IDX];
        break;
      }
    case EmwApiBase::eWIFI_EVENT_AP_UP:
    case EmwApiBase::eWIFI_EVENT_AP_DOWN: {
        wifi_interface = EmwApiBase::eSOFTAP;
        status_callback_function = THIS->callbacks.wiFiStatusCallbacks[EmwApiBase::eWIFI_INTERFACE_SOFTAP_IDX];
        callback_arg = THIS->callbacks.wiFiStatusCallbackArgPtrs[EmwApiBase::eWIFI_INTERFACE_SOFTAP_IDX];
        break;
      }
  ...
  if (status_callback_function != nullptr) {
    ...
    DEBUG_API_LOG("\n     EmwApiCore::ProcessWiFiStatusEvent()<\n")
  }
}

#if defined(EMW_NETWORK_BYPASS_MODE)
void EmwApiCore::ProcessWiFiNetlinkInput(const EmwApiCore *THIS, EmwNetworkStack::Buffer_t *networkBufferPtr) noexcept
{
  std::uint8_t (&payload)[] \
    = reinterpret_cast<std::uint8_t (&)[]>(* EmwNetworkStack::GetBufferPayload(networkBufferPtr));
  const WiFiBypassInParams_t *const parameters_ptr \
    = reinterpret_cast<const WiFiBypassInParams_t *>(EmwCoreIpc::SkipHeader(payload));

  EMW_STATS_INCREMENT(callback)
  DEBUG_API_LOG("\n     EmwApiCore::ProcessWiFiNetlinkInput() %p>\n", static_cast<const void *>(networkBufferPtr));

  if ((THIS->callbacks.netlinkInputCallback != nullptr) && (parameters_ptr->totalLength > 0U)) {
    ...
    THIS->callbacks.netlinkInputCallback(networkBufferPtr, low_level_netif_idx);
  }
  else {
    EmwNetworkStack::FreeBuffer(networkBufferPtr);
  }
...
volatile std::uint8_t EmwApiCore::interfaces = 0U;
