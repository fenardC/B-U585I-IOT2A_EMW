...
static bool IsTimedOut(std::uint32_t tickStart, std::uint32_t tickCount);
...
#endif /* COMPILATION_WITH_FREERTOS) */
      if (IsTimedOut(tick_start, 10000U)) {
        STD_PRINTF(" Not connected in the 10 last seconds.\n");
        break;
      }
    }
  }
  STD_PRINTF("\n Connected: %s\n", Emw.isConnected() == 1 ? "true" : "false");
  if (Emw.isConnected() == 1) {
    {
      std::uint8_t ip_address_bytes[4] = {0U, 0U, 0U, 0U};
                                  ...
                                  enum EmwApiBase::WiFiEvent status, void *argumentPtr) noexcept
{
  ...
  if (interface == EmwApiBase::eSTATION) {
    switch (status) {
  ...
  else if (interface == EmwApiBase::eSOFTAP) {
    switch (status) {
      case EmwApiBase::eWIFI_EVENT_AP_DOWN: {
          AppWiFiEmw::EmwInterfaceSoftApUp = false;
