...
static std::uint32_t ConvertToOsTicks(uint32_t milliseconds) noexcept;
...
void AppHttpSSE::initializeServer(std::uint32_t ipAddressIn, std::uint16_t port) noexcept
{
  ...
  AppHttpSSE::serverPort = port;
  {
    ...
    static const char webserver_task_name[] = {"HTTP-Main"};

    if (xTaskCreate(AppHttpSSE::doListenService, webserver_task_name,
                    AppHttpSSE::WEBSERVER_TASK_STACK_SIZE, this,
                    AppHttpSSE::WEBSERVER_TASK_PRIORITY, NULL) != PASS) {
      ErrorHandler();
    }
  }
}
...
void AppHttpSSE::doAcceptService(void *argPtr) noexcept
{
  ...
  try {
    ...
    {
      struct sockaddr_in sock_address_name_in {};
      uint32_t sock_addr_name_in_size = sizeof(sock_address_name_in);

      sock_address_name_in.sin_len = sizeof(sock_address_name_in);
      sock_address_name_in.sin_family = AF_INET;
      if (lwip_getpeername(peer.sock, reinterpret_cast<struct sockaddr *>(&sock_address_name_in),
                           &sock_addr_name_in_size) >= 0) {
        peer.setInfo(sock_address_name_in.sin_addr.s_addr, sock_address_name_in.sin_port);
      }
    }
    peer.infos();
    {
      ...
      if (count > 0) {
        ...
        peer.close();
        peer.infos();
        STD_PRINTF(" AppHttpSSE::doAcceptService(): %" PRIi32 "\n", count);
      ...
      peer.infos();
      while (peer.getStatus() == AppHttpSSE::Client::eSTATUS_COMM_STREAM) {
        peer.infos();
        vTaskDelay(ConvertToOsTicks(500U));
        {
    ...
    STD_PRINTF(" AppHttpSSE::doAcceptService()< %s\n", error.what());
    STD_PRINTF(" AppHttpSSE::doAcceptService()< errno  <%" PRIi32 ">\n", static_cast<std::int32_t>(errno));
    STD_PRINTF(" AppHttpSSE::doAcceptService()< socket <%" PRIi32 ">\n", sock);
  }
  peer.close();
  lwip_socket_thread_cleanup();

  DEBUG_STD_PRINTF(" AppHttpSSE::doAcceptService()< leaving thread #<%" PRIi32 ">\n", sock)
...
{
  DEBUG_STD_PRINTF(" AppHttpSSE::doListenService()>\n")
  ...
  try {
    ...
    }
    {
      ...
      {{this->lwipNetif.name[0], this->lwipNetif.name[1], static_cast<char>('0' + this->lwipNetif.num), '\0', 0, 0}};

      if (lwip_setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE, &iface, sizeof(iface)) != 0) {
    ...
    }
    {
      ...
      sock_address_in.sin_port = lwip_htons(AppHttpSSE::serverPort);
      sock_address_in.sin_addr.s_addr = AppHttpSSE::serverIpAddrIn;
      if (lwip_bind(sock, reinterpret_cast<struct sockaddr *>(&sock_address_in), sizeof(sock_address_in)) != 0) {
        throw std::runtime_error("lwip_bind() failed");
      }
    }
    if (lwip_listen(sock, AppHttpSSE::WEBSERVER_LISTEN_BACKLOG) != 0) {
      throw std::runtime_error("lwip_listen() failed");
    }

    while (true) {
      std::int32_t connected_sock;
      struct sockaddr_in sock_address_remote_in {};
      std::uint32_t sock_addr_remote_in_size = sizeof(sock_address_remote_in);

      sock_address_remote_in.sin_len = sizeof(sock_address_remote_in);
      sock_address_remote_in.sin_family = AF_INET;

      connected_sock = lwip_accept(sock, reinterpret_cast<struct sockaddr *>(&sock_address_remote_in),
                                   &sock_addr_remote_in_size);
      if (connected_sock < 0) {
...

      {
        ...
        static_cast<void>(std::snprintf(task_name, task_name_length - 1, "HTTP-%04" PRIX32, connected_sock));
        task_name[task_name_length - 1] = '\0';
        if (xTaskCreate(doAcceptService, task_name,
                        AppHttpSSE::WEBSERVER_CHILD_TASK_STACK_SIZE, static_cast<void *>(&connected_sock),
                        AppHttpSSE::WEBSERVER_CHILD_TASK_PRIORITY, NULL) != PASS) {
    ...
    STD_PRINTF(" AppHttpSSE::doListenService()< %s\n", error.what());
...
void AppHttpSSE::listRemainingFreeHeap(char *bufferStringPtr, std::size_t bufferStringSize) noexcept
{
  ...
  static_cast<void>(std::snprintf(free_heap, free_heap_size, "Remaining heap: %08" PRId32,
...
char *AppHttpSSE::chop(char *stringPtr) noexcept
{
  ...
  while ((stringPtr != char_ptr) && ((*char_ptr == '\n') || (*char_ptr == '\r'))) {
    *char_ptr = '\0';
    --char_ptr;
  }
  return stringPtr;
                                    ...
                                    char *messagePtr) noexcept
{
...

  {
    ...
    chunked_size += written;
    {
      ...
      static_cast<void>(std::snprintf(chunked_size_string, 4, "%3" PRIx32 "", chunked_size));
...
std::int32_t AppHttpSSE::Client::sendEvent(const char *eventNamePtr, char *bufferStringPtr) noexcept
{
  ...
  if (this->status == AppHttpSSE::Client::eSTATUS_COMM_STREAM) {
    ...
    this->message++;
    DEBUG_STD_PRINTF(" ->Send to %s <%" PRIi32 ">\n", this->info, this->sock)
    ...
    if (this->send(send_outbuffer, strlen(send_outbuffer))) {
      if (this->err) {
        DEBUG_STD_PRINTF(" Close client which signed error at send:\n")
        this->close();
      }
    }
  }
...
std::int32_t AppHttpSSE::Client::send(const char *bufferPtr, std::size_t bufferSize) noexcept
{
  ...
  DEBUG_STD_PRINTF("[%" PRIi32 "] *****%s*****\n", this->sock, bufferPtr)

  if (this->err) {
    return 1;
  }
  if (lwip_send(this->sock, bufferPtr, bufferSize, 0) != to_send) {
    this->err = 1;
    DEBUG_STD_PRINTF("[%" PRIi32 "] ***** ERROR *****\n", this->sock)
    return 1;
  }
  return 0;
}
...
void AppHttpSSE::Client::setInfo(std::uint32_t address, std::uint16_t port) noexcept
{
  ...
  static_cast<void>(std::snprintf(this->info, sizeof(this->info) - 1,
...
void AppHttpSSE::Client::infos() const noexcept
{
  ...
  STD_PRINTF(" #<%" PRIi32 "> info: %s S: %s Err: %d Messages: %" PRIi32 "\n",
...
void AppHttpSSE::Client::close(void) noexcept
{
  ...
  DEBUG_STD_PRINTF(" !!! Closed connection <%" PRIi32 "> !!!\n", this->sock)
}
...
void AppHttpSSE::Client::sendIndexHtml(void) noexcept
{
  static const char response_index_html[] = {
    ...
    "</div>"
    "</div>"
    "<div class=\"footer\"><p>Cyril FENARD, 2025-2026</p></div>"
    "<script>"
...
void AppHttpSSE::Client::sendEventStream(void) noexcept
{
  ...
  if (this->send(response_event_stream, strlen(response_event_stream)) == 0) {
