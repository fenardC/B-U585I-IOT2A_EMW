  ...
  if (argc == 1) {
    ...
    for (i = 1; i < argc; i++) {
      if (nullptr != argvPtrs[i]) {
        ...
        else if (0 == strncmp("-c", argvPtrs[i], 2)) {
...
int32_t AppConsoleEcho::doEcho6(const struct sockaddr_in6 &sAddressIn6, uint32_t loop, uint32_t untilN)
{
  int32_t status = 0;
  ...
  try {
    sock = lwip_socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
    if (-1 == sock) {
      throw std::runtime_error("lwip_socket() failed");
    }
    {
      ...
      for (uint32_t i = 0; i < loop; i++) {
        {
          ...
          do {
            int32_t count_done = lwip_send(sock, &this->bufferOut[i + transfer_out], untilN - transfer_out, 0);
            std::printf(".");
            if (count_done < 0) {
        ...
        memset(this->bufferIn, 0x00, sizeof(this->bufferIn));
        {
          ...
          do {
            int32_t count_done = lwip_recv(sock, &this->bufferIn[transfer_in], untilN - transfer_in, 0);
            std::printf("x");
            if (count_done < 0) {
