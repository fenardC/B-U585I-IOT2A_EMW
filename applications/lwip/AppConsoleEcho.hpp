...
class AppConsoleEcho final : public Cmd {
  ...
  private:
    std::int32_t doEcho(const struct sockaddr_in &sAddressIn, std::uint32_t loop, std::uint32_t untilN) noexcept;
  private:
    std::int32_t doEcho6(const struct sockaddr_in6 &sAddressIn6, std::uint32_t loop, std::uint32_t untilN) noexcept;
  private:
    std::int32_t doEchoExchanges(std::int32_t sock, std::uint32_t loop, std::uint32_t untilN) noexcept;
  private:
