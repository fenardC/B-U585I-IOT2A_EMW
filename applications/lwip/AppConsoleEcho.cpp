                                    ...
                                    std::uint32_t untilN) noexcept
{
  ...
  try {
    ...
    }
    {
      ...
      std::int32_t \
      socket_status = lwip_setsockopt(this->socket, SOL_SOCKET, SO_RCVTIMEO, &timeout_in_ms, sizeof(timeout_in_ms));

      if (socket_status == -1) {
        throw std::runtime_error("lwip_setsockopt() for receiving failed");
      }
      socket_status = lwip_setsockopt(this->socket, SOL_SOCKET, SO_SNDTIMEO, &timeout_in_ms, sizeof(timeout_in_ms));
      if (socket_status == -1) {
    ...
    status = this->doEchoExchanges(loop, untilN);
  }
                                     ...
                                     std::uint32_t untilN) noexcept
{
  ...
  try {
    this->socket = lwip_socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
    if (this->socket == -1) {
      throw std::runtime_error("lwip_socket() failed");
    }
    {
...
std::int32_t AppConsoleEcho::doEchoExchanges(std::uint32_t loop, std::uint32_t untilN) noexcept
{
  std::int32_t status = 0;
      ...
      for (std::uint32_t i = 0U; i < loop; i++) {
        {
          ...
          do {
            std::int32_t count_done = lwip_send(this->socket,
        ...
        std::memset(this->bufferIn, 0x00, sizeof(this->bufferIn));
        {
          ...
          do {
            std::int32_t count_done = lwip_recv(this->socket,
  ...
  static_cast<void>(lwip_shutdown(this->socket, SHUT_RDWR));
  static_cast<void>(lwip_close(this->socket));
