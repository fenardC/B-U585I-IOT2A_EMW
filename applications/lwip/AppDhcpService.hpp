...
class AppDhcpService final {
  ...
  private:
    typedef struct DhcpAddress_s {
      constexpr DhcpAddress_s(void) : ipAddress(0U), clientIdentifierPtr(nullptr), clientIdentifierSize(0U) {}
      uint32_t ipAddress;
  ...
  private:
    static bool checkAddressValue(const AppDhcpService::DhcpAddress_t &addressInfo, const void *keyPtr);
  private:
    static bool checkAddressInfo(const AppDhcpService::DhcpAddress_t &addressInfo, const void *keyPtr);
  private:
  ...
  private:
    void sendReply(uint8_t reply_buffer[], uint32_t reply_buffer_size,
                   AppDhcpService::DhcpMessageTypes message_type, uint32_t giaddr,
  ...
  private:
    static const uint32_t LWIP_INADDR_BROADCAST = 0xFFFFFFFF;
  private:
    static const uint32_t LWIP_IPADDR_LOOPBACK = 0x7F000001;
  private:
