...
class AppConsolePing final : public Cmd {
  ...
  private:
    int32_t icmpPing(const struct netif *netifPtr, struct sockaddr *s_addrPtr, int32_t count, uint32_t timeoutInMs,
                     int32_t responses[]);
  private:
    void preparePingEcho(struct icmp_echo_hdr *dataPtr, uint16_t length, uint16_t sequenceNumber);
