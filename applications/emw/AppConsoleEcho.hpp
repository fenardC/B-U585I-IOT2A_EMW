...
class AppConsoleEcho final : public Cmd {
  ...
  private:
    std::int32_t doEcho(const EmwAddress::SockAddrIn_t &sockAddressIn, std::uint32_t loop, std::uint32_t untilN) noexcept;
  private:
    std::int32_t doEcho6(const EmwAddress::SockAddrIn6_t &sockAddressIn6, std::uint32_t loop,
                         std::uint32_t untilN) noexcept;
  private:
    std::int32_t doEchoExchanges(std::uint32_t loop, std::uint32_t untilN) noexcept;
  private:
