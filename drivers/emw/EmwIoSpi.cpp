        ...
        if (0 == this->exchangeHeaders(EmwIoSpi::txDataLength, &rx_length)) {
          if (EmwNetworkStack::getBufferPayloadSize(network_buffer_ptr) < rx_length) {
            DEBUG_IO_LOG("EmwIoSpi::processPollingData(): length: %" PRIu32 "-%" PRIu32 "\n",
                         (uint32_t)rx_length, (uint32_t)EmwIoSpi::txDataLength)
...
HAL_StatusTypeDef EmwIoSpi::receive(uint8_t *rxDataPtr, uint16_t rxDataLength)
{
  ...
  ret = HAL_SPI_Receive_DMA(this->configuration.hSpiPtr, rxDataPtr, (uint32_t)rxDataLength);
  ...
  for (uint32_t i = 0U; i < rxDataLength; i++) {
    DEBUG_IO_LOG("%02" PRIx32 " ", (uint32_t)(*(rxDataPtr + i)))
  }
...
HAL_StatusTypeDef EmwIoSpi::transmit(const uint8_t *txDataPtr, uint16_t dataLength)
{
  ...
  for (uint32_t i = 0U; i < dataLength; i++) {
    DEBUG_IO_LOG("%02" PRIx32 " ", (uint32_t)(*(txDataPtr + i)))
  }
#endif /* DEBUG_DETAILS_IO_LOG */
  ret = HAL_SPI_Transmit_DMA(this->configuration.hSpiPtr, txDataPtr, (uint32_t)dataLength);
  (void) EmwOsInterface::takeSemaphore(&EmwIoSpi::transferDoneSem, EmwIoSpi::TIMEOUT_HARDWARE_EMW_MS);
...
#endif /* DEBUG_DETAILS_IO_LOG */
  ret = HAL_SPI_TransmitReceive_DMA(this->configuration.hSpiPtr, txDataPtr, pRxData, (uint32_t)dataLength);
  (void) EmwOsInterface::takeSemaphore(&EmwIoSpi::transferDoneSem, EmwIoSpi::TIMEOUT_HARDWARE_EMW_MS);
...
int32_t EmwIoSpi::exchangeHeaders(uint16_t txLength, uint16_t *rxLengthPtr)
{
  ...
  const spi_header_t spi_master_header = {SPI_WRITE, txLength, (uint16_t)~txLength, {0U, 0U, 0U}};
  spi_header_t spi_slave_header = {0U, 0U, 0U, {0U, 0U, 0U}};
  DEBUG_IO_LOG("\nEmwIoSpi::exchangeHeaders()>\n")
  if (HAL_OK != this->transmitReceive(reinterpret_cast<const uint8_t *>(&spi_master_header),
                                      reinterpret_cast<uint8_t *>(&spi_slave_header),
                                      (uint16_t)sizeof(spi_master_header))) {
  ...
  else if (((uint16_t)UINT16_MAX) != ((uint16_t)(spi_slave_header.len ^ spi_slave_header.lenx))) {
    DEBUG_IO_LOG("EmwIoSpi::ExchangeHeaders(): length %04" PRIx32 "-%04" PRIx32 "\n",
                 (uint32_t)spi_slave_header.len, (uint32_t)spi_slave_header.lenx)
...
void EmwIoSpi::start(const class EmwApiCore &core)
{
  ...
  }
  {
    ...
    const EmwOsInterface::Status os_status = EmwOsInterface::createSemaphore(&EmwIoSpi::fowRiseSem, flow_rise_sem_name, 1U,
        0U);
    EmwOsInterface::assertAlways(EmwOsInterface::eOK == os_status);
  }
  {
...
#if defined(EMW_WITH_RTOS)
  {
    ...
    const EmwOsInterface::Status os_status = EmwOsInterface::createThread(&IoThread, io_thread_name,
        IoThreadFunction, reinterpret_cast<EmwOsInterface::thread_function_argument>(&core),
...
void EmwIoSpi::IoThreadFunction(EmwOsInterface::thread_function_argument argument)
{
  const class EmwApiCore *const core_ptr = reinterpret_cast<const class EmwApiCore *>(argument);
#if defined(EMW_IO_DEBUG)
