...
class EmwApiEmw final : public EmwApiCore {
  ...
  public:
    std::int32_t socketPing(const char (&hostnameString)[16],
                            std::int32_t count, std::int32_t delayInMs, std::int32_t (&responses)[10]) noexcept;
  public:
    std::int32_t socketPing6(const char (&hostnameString)[40],
                             std::int32_t count, std::int32_t delayInMs, std::int32_t (&responses)[10]) noexcept;
  ...
  public:
    std::uint32_t tlsConnect(std::int32_t domain, std::int32_t type, std::int32_t protocol,
  ...
  public:
    std::uint32_t tlsConnectSni(const char (&serverNameInformationString)[128], std::int32_t serverNameInformationLength,
                                const EmwAddress::SockAddrStorage_t &socketAddress, std::int32_t socketAddressSize,
                                const char (&caString)[2500], std::int32_t caStringLength) noexcept;
  public:
    std::int32_t tlsSend(EmwApiBase::Mtls_t tlsKey, const std::uint8_t (&data)[], std::size_t dataLength) noexcept;
  public:
    std::int32_t tlsReceive(EmwApiBase::Mtls_t tlsKey, std::uint8_t (&data)[], std::int32_t dataLength) noexcept;
  public:
    std::int32_t tlsClose(EmwApiBase::Mtls_t tlsKey) noexcept;
  public:
    std::int32_t tlsSetNonBlocking(EmwApiBase::Mtls_t tlsKey, std::int32_t nonblock) noexcept;
