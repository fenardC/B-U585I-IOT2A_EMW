...
#define CMD_BASE (static_cast<std::uint16_t>(ID_NONE))
  ...
  protected:
    Status testEcho(std::uint8_t (&dataIn)[], std::uint16_t dataInLength,
  ...
  protected:
    ...
    typedef __PACKED_STRUCT TlsAcceptResponseParams_s {
      constexpr TlsAcceptResponseParams_s(void) noexcept : tlsKey(0U) {}
      EmwApiBase::Mtls_t tlsKey;
    ...
    typedef __PACKED_STRUCT TlsConnectSniResponseParams_s {
      constexpr TlsConnectSniResponseParams_s(void) noexcept : tlsKey(0U), emwErrno(0) {}
      EmwApiBase::Mtls_t tlsKey;
      std::int32_t emwErrno;
    } TlsConnectSniResponseParams_t;

    typedef __PACKED_STRUCT TlsSendParams_s {
      constexpr TlsSendParams_s(void) noexcept : tlsKey(0U), size(0), buffer{0U} {}
      EmwApiBase::Mtls_t tlsKey;
      std::size_t size;
      std::uint8_t buffer[1];
    } TlsSendParams_t;
    ...
    typedef __PACKED_STRUCT TlsCloseParams_s {
      constexpr TlsCloseParams_s(void) noexcept
        : tlsKey(0U) {}
      constexpr explicit TlsCloseParams_s(EmwApiBase::Mtls_t tlsKey) noexcept
        : tlsKey(tlsKey) {}
      EmwApiBase::Mtls_t tlsKey;
    } TlsCloseParams_t;
    ...
    typedef __PACKED_STRUCT TlsSetNonblockParams_s {
      constexpr TlsSetNonblockParams_s(void) noexcept
        : tlsKey(0U), nonblock(-1) {}
      constexpr explicit TlsSetNonblockParams_s(EmwApiBase::Mtls_t tlsKey, std::int32_t nonblock) noexcept
        : tlsKey(tlsKey), nonblock(nonblock) {}
      EmwApiBase::Mtls_t tlsKey;
...
#define MIPC_API_EVENT_BASE (static_cast<std::uint16_t>(0x8000U))
  ...
  private:
    static void processResponse(EmwNetworkStack::Buffer_t *networkBufferPtr, std::uint32_t reqId,
                                std::uint8_t *payloadPtr, std::uint32_t payloadSize) noexcept;
  protected:
    static const std::uint32_t EMW_CMD_TIMEOUT = 10000U;
