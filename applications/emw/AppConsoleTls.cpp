...
std::int32_t AppConsoleTls::doTls(const char (&peerNameString)[128], bool checkCa) noexcept
{
  ...
  try {
    if (Emw.socketGetHostByName(reinterpret_cast<EmwAddress::SockAddr_t &>(sock_address_in),
                                      reinterpret_cast<const char(&)[255]>(peerNameString)) < 0) {
    ...
    }
    {
      ...
      std::strncpy(server_name_information_string, peerNameString, 127);
      {
        ...
        if (this->doTlsExchanges(tls_key) != 0) {
          static_cast<void>(Emw.tlsClose(tls_key));
          throw std::runtime_error("doTlsExchanges() failed");
        }

        if (Emw.tlsClose(tls_key) == 0) {
          STD_PRINTF(" %s: closed\n", this->getName());
        }
      }
    }
...
std::int32_t AppConsoleTls::doTlsExchanges(EmwApiBase::Mtls_t tlsMagic) noexcept
{
  ...
  try {
...

    {
      ...
      response[0] = '\0';
      response_count = Emw.tlsReceive(tlsMagic, response, max_length);
      if (response_count <= 0) {
      ...
      if (date_string_start_ptr == nullptr) {
