...
std::int32_t AppConsoleDownload::doDownload(const char *hostPtr, const char *requestPtr) noexcept
{
  ...
  struct sockaddr * const sock_address_ptr = reinterpret_cast<struct sockaddr *>(&sock_address_in);

  DEBUG_STD_PRINTF("\n AppConsoleDownload::doDownload()>\n")

  sock_address_in.sin_len = sizeof(sock_address_in);
  sock_address_in.sin_family = AF_INET;
  sock_address_in.sin_port = lwip_htons(AppConsoleDownload::REMOTE_TCP_PORT);

  if (AppLwipService::getHostByName(sock_address_ptr, hostPtr) < 0) {
    STD_PRINTF(" %s: failed to find the host name \"%s\"\n", this->getName(), hostPtr);
  }
  else {
    ip_addr_t ip_address = IPADDR4_INIT(0);
    char ip4_address_string[16] = {"000.000.000.000"};
    inet_addr_to_ip4addr(&ip_address.u_addr.ip4, &sock_address_in.sin_addr);
    ip4addr_ntoa_r(&ip_address.u_addr.ip4, ip4_address_string, sizeof(ip4_address_string));

    STD_PRINTF(" %s: downloading \"%s\" %" PRIu32 " with \"%s\"\n",
               this->getName(), hostPtr, this->userDownloadLength, ip4_address_string);
    try {
      this->socket = lwip_socket(sock_address_ptr->sa_family, SOCK_STREAM, IPPROTO_TCP);
      if (this->socket == -1) {
      ...
      {
        const std::int32_t connect_status = lwip_connect(this->socket,
                                            sock_address_ptr,
                                            sock_address_ptr->sa_len);
        if (connect_status != 0) {
      ...
      status = -1;
      if (this->socket >= 0) {
        static_cast<void>(lwip_close(this->socket));
      }
    }
  }
  DEBUG_STD_PRINTF("\n AppConsoleDownload::doDownload()<\n")
...
std::int32_t AppConsoleDownload::doDownload6(const char *hostPtr, const char *requestPtr) noexcept
{
  ...
  struct sockaddr * const sock_address_ptr = reinterpret_cast<struct sockaddr *>(&sock_address_in6);

  DEBUG_STD_PRINTF("\n AppConsoleDownload::doDownload6()>\n")

  sock_address_in6.sin6_len = sizeof(sock_address_in6);
  sock_address_in6.sin6_family = AF_INET6;
  sock_address_in6.sin6_port = lwip_htons(AppConsoleDownload::REMOTE_TCP_PORT);

  if (AppLwipService::getHostByName(sock_address_ptr, hostPtr) < 0) {
    STD_PRINTF(" %s: failed to find the host name \"%s\"\n", this->getName(), hostPtr);
  }
  else {
    ip_addr_t ip_address = IPADDR6_INIT(0, 0, 0, 0);
    char ip6_address_string[40] = {"0000:0000:0000:0000:0000:0000:0000:0000"};

    inet6_addr_to_ip6addr(&ip_address.u_addr.ip6, &sock_address_in6.sin6_addr);
    ip6addr_ntoa_r(&ip_address.u_addr.ip6, ip6_address_string, sizeof(ip6_address_string));
    STD_PRINTF(" %s: downloading \"%s\" %" PRIu32 " with \"%s\"\n",
    ...
    const char *requestPtr) const noexcept
{
  ...
  buffer_ptr(static_cast<std::uint8_t *>(pvPortCalloc(buffer_size, sizeof(std::uint8_t))), &vPortFree);
  ...
  try {
    ...
    *(buffer_ptr.get() + buffer_size - 1) = '\0';
    ...
    while (length) {
      ...
      const std::int32_t count = lwip_recv(this->socket,
                                           buffer_ptr.get(),
                                           size_in_bytes, 0);
      if (count == -1) {
        break;
    ...
    static_cast<void>(lwip_shutdown(this->socket, SHUT_RD));
    static_cast<void>(lwip_close(this->socket));
    {
      const std::uint32_t duration_in_ms = HAL_GetTick() - start_time_in_ms;

      if (duration_in_ms != 0U) {
    ...
    std::uint32_t &retSize) const noexcept
{
  ...
  while (header_size > 0) {
    const std::int32_t ret = lwip_recv(this->socket,
                                       string_ptr, 1, 0);
    if (ret < 0) {
      return ret;
    }
...
static void SplitHostRequest(const char *urlStringPtr,
                             char (&hostString)[255], const char * &requestStringPtr) noexcept
{
  if ((urlStringPtr == nullptr) || (*urlStringPtr == '\0')) {
    hostString[0] = '\0';
