...
static const std::uint8_t SDHCP_BOOT_REQUEST = 1U;
static const std::uint8_t SDHCP_BOOT_REPLY = 2U;
static const std::uint32_t SDHCP_ADDRESS_BROADCAST = 0xFFFFFFFFU;
static const std::uint32_t SDHCP_ADDRESS_LOOPBACK = 0x7F000001U;
static const std::size_t SDHCP_MAX_HOSTNAME_LENGTH = 64U;
static const std::uint8_t SDHCP_MAGIC_COOKIE[4] = {99U, 130U, 83U, 99U}; /* MSB first. */
static const std::uint16_t SDHCP_NET_DHCP_MSG_OFFSET_FLAGS = 10U;
static const std::uint16_t SDHCP_CLIENT_PORT = 68U;
static const std::uint16_t SDHCP_SERVER_PORT = 67U;
...
#define MAX_UDP_MESSAGE_SIZE  (sizeof(AppDhcpService::DhcpMessage_t) + sizeof(SDHCP_MAGIC_COOKIE) + 700)
...
bool AppDhcpService::CheckHardwareAddress(const AppDhcpService::DhcpAddress_t &addressInfo, const void *keyPtr) noexcept
{
  ...
  SDHCP_ASSERT(client_ptr != nullptr);
                                ...
                                std::uint8_t *&dataPtr, uint32_t &dataLength) noexcept
{
  ...
  SDHCP_ASSERT((optionsLength == 0) || (optionsPtr != nullptr));
  ...
  while (current_ptr < end_ptr) {
    ...
    if (current == AppDhcpService::ePAD) {
      continue;
    }
    if (current == AppDhcpService::eEND) {
      break;
    }
                                    ...
                                    AppDhcpService::DhcpMessageTypes &messageType) noexcept
{
  ...
  if (this->findOption(AppDhcpService::eDHCPMESSAGETYPE, optionsPtr, optionsLength,
    ...
    if ((dhcp_message_length == 1U) && (type >= AppDhcpService::eTYPE_FIRST_VALUE) \
...
bool AppDhcpService::initializeService(void) noexcept
{
  ...
  if (this->sock >= 0) {
    ...
    socket_address_in.sin_port = lwip_htons(SDHCP_SERVER_PORT);
    {
      const struct ifreq iface = {
        {
          this->netifPtr->name[0], this->netifPtr->name[1],
          static_cast<char>('0' + this->netifPtr->num), '\0', 0, 0
        }
      };
      static_cast<void>(lwip_setsockopt(this->sock, SOL_SOCKET, SO_BINDTODEVICE, &iface, sizeof(iface)));
    }
    if (lwip_bind(this->sock,
                  reinterpret_cast<struct sockaddr *>(&socket_address_in), sizeof(socket_address_in)) != -1) {
      {
        std::int32_t broadcast = static_cast<std::int32_t>(true);
        if (lwip_setsockopt(this->sock, SOL_SOCKET, SO_BROADCAST, &broadcast, sizeof(broadcast)) == 0) {
          status_done = true;
        }
        else {
      ...
      STD_PRINTF(" SDHCP: Unable to bind to server socket (port %" PRIu32 ")\n",
                 static_cast<std::uint32_t>(SDHCP_SERVER_PORT));
    }
    if (!status_done) {
...
#define HARDWARE_ADDRESS_LENGTH sizeof((static_cast<AppDhcpService::DhcpAddress_s *>(nullptr))->hardwareAddress)
  ...
  if ((clientHardwareAddressPtr != nullptr) \
      && (clientHardwareAddressLength != 0U) \
      && (clientHardwareAddressLength <= HARDWARE_ADDRESS_LENGTH)) {
    ...
    if (isAddressSeenBefore) {
      ...
      if ((this->lastOfferIpAddress < this->MINIMUM_IP_ADDRESS) || (this->lastOfferIpAddress > this->MAXIMUM_IP_ADDRESS)) {
        new_offer_addr = this->MINIMUM_IP_ADDRESS;
      }
      else if (this->lastOfferIpAddress == this->MAXIMUM_IP_ADDRESS) {
        new_offer_addr = this->MINIMUM_IP_ADDRESS;
      }
      else {
      ...
      for (std::uint32_t i = 0U; i < pool_size; ++i) {
        ...
        if (new_offer_addr == this->MAXIMUM_IP_ADDRESS) {
          new_offer_addr = this->MINIMUM_IP_ADDRESS;
        }
        else {
          ++new_offer_addr;
    ...
    AppDhcpService::DhcpMessage_t &reply, AppDhcpService::DhcpMessageTypes &replyOptionType) noexcept
{
  ...
  if (reqServerIdFound) {
    ...
    if ((requestedIpAddress != SDHCP_ADDRESS_BROADCAST) || (clientIpAddress != 0U)) {
  ...
  if (reply_option_type == AppDhcpService::eTYPE_ACK) {
    SDHCP_ASSERT(SDHCP_ADDRESS_BROADCAST != offerAddress);
...
void AppDhcpService::processRequest(const std::uint8_t requestData[], std::int32_t requestDataLength) noexcept
{
  ...
  const std::int32_t request_minimal_size \
    = static_cast<std::int32_t>(sizeof(AppDhcpService::DhcpMessage_t) + sizeof(SDHCP_MAGIC_COOKIE));
  ...
  if ((requestDataLength >= request_minimal_size) && (request_ptr->op == SDHCP_BOOT_REQUEST) \
      && (0 == std::memcmp(SDHCP_MAGIC_COOKIE, &request_ptr->file[128], sizeof(SDHCP_MAGIC_COOKIE)))) {
    const std::uint8_t *const request_options_ptr \
      = reinterpret_cast<const std::uint8_t *>(&request_ptr->file[128]) + sizeof(SDHCP_MAGIC_COOKIE);
    ...
    if (this->getMessageType(request_options_ptr, request_options_size, client_message_type)) {
      char host_name[SDHCP_MAX_HOSTNAME_LENGTH] = {0};
      {
      ...
      if (std::strncmp(host_name, this->hostNamePtr, strlen(this->hostNamePtr)) != 0) {
        ...
        std::uint32_t offer_addr = SDHCP_ADDRESS_BROADCAST;
        ...
        switch (client_message_type) {
          ...
          case AppDhcpService::eTYPE_REQUEST: {
              std::uint32_t requested_address = SDHCP_ADDRESS_BROADCAST;
              {
                ...
                if (findOption(AppDhcpService::eREQUESTEDIPADDRESS, request_options_ptr, request_options_size,
                  ...
                  if (address_size == sizeof(requested_address)) {
                    /* MSB first */
                               ...
                               std::uint32_t clientIpAddress, std::uint32_t yourIpAddress, std::uint16_t requestedFlag) noexcept
{
  ...
  if (gatewayIpAddress == 0U) {
    ...
    replyBuffer[SDHCP_NET_DHCP_MSG_OFFSET_FLAGS] |= BROADCAST_FLAG;
  }

  SDHCP_ASSERT((peer_addr != SDHCP_ADDRESS_LOOPBACK) && (peer_addr != 0U));
  {
    ...
    sock_address_in.sin_port = lwip_htons(SDHCP_CLIENT_PORT);
...
{
  replyServerOptions.magic[0] = SDHCP_MAGIC_COOKIE[0];
  replyServerOptions.magic[1] = SDHCP_MAGIC_COOKIE[1];
  replyServerOptions.magic[2] = SDHCP_MAGIC_COOKIE[2];
  replyServerOptions.magic[3] = SDHCP_MAGIC_COOKIE[3];
