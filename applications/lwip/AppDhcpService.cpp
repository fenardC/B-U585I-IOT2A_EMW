...
bool AppDhcpService::checkIpAddress(const AppDhcpService::DhcpAddress_t &addressInfo, const void *keyPtr)
{
...
bool AppDhcpService::checkHardwareAddress(const AppDhcpService::DhcpAddress_t &addressInfo, const void *keyPtr)
{
...
int32_t AppDhcpService::handleDiscoverMessage(bool isAddressSeenBefore, uint32_t offerAddress,
    uint8_t *clientHardwareAddressPtr, uint32_t clientHardwareAddressLength,
...
int32_t AppDhcpService::handleRequestMessage(uint32_t clientIpAddress,
    ...
    AppDhcpService::DhcpMessage_t &reply, AppDhcpService::DhcpMessageTypes &replyOptionType)
{
  ...
  if (reqServerIdFound && (this->hostIpAddress == reqServer)) {
    ...
    if ((AppDhcpService::ADDRESS_BROADCAST != requestedIpAddress) \
        || ((AppDhcpService::ADDRESS_BROADCAST == requestedIpAddress) && (0U != clientIpAddress))) {
  ...
  if ((request_minimal_size <= requestDataLength) && (AppDhcpService::BOOTREQUEST == request_ptr->op) \
    ...
    if (this->getMessageType(request_options_ptr, request_options_size, client_message_type)) {
      ...
      if (0 != std::strncmp(host_name, this->hostNamePtr, strlen(this->hostNamePtr))) {
        ...
        uint32_t offer_addr = AppDhcpService::ADDRESS_BROADCAST;
        uint8_t *client_hardware_address_ptr = nullptr;
        uint32_t client_hardware_address_length = 0U;
        ...
        if (!findOption(AppDhcpService::eCLIENTIDENTIFIER, request_options_ptr, request_options_size,
          ...
          client_hardware_address_ptr = const_cast<uint8_t *>(&request_ptr->chaddr[0]);
          client_hardware_address_length = sizeof(request_ptr->chaddr);
        }
        {
          AppDhcpService::ClientHardwareAddress_t client = {client_hardware_address_ptr, static_cast<uint32_t>(client_hardware_address_length)};
          const int32_t index = findIndex(this->addressInfos, this->addressInfosStored, AppDhcpService::checkHardwareAddress,
                                          &client);
        ...
        switch (client_message_type) {
          ...
          case AppDhcpService::eTYPE_REQUEST: {
              uint32_t requested_address = AppDhcpService::ADDRESS_BROADCAST;
              {
...
{
  uint32_t peer_addr = AppDhcpService::ADDRESS_LOOPBACK;
