...
EmwCoreIpc::Status EmwCoreIpc::request(const class EmwApiCore &core, uint8_t *commandPtr, uint16_t commandSize,
                                       uint8_t *responseBufferPtr, uint16_t *responseBufferSize,
                                       uint32_t timeoutInMs)
{
  EmwCoreIpc::Status ret = EmwCoreIpc::eERROR;

  if ((0U < core.runtime.interfaces) && (nullptr != commandPtr) && (nullptr != responseBufferSize)) {
    ...
    if (is_command_size_allowed) {
      ...
      EmwCoreIpc::PendingRequest.responseBufferSizePtr = responseBufferSize;
      ...
      DEBUG_IPC_LOG("EmwCoreIpc::request(): req_id: 0x%08" PRIx32 " : %" PRIu32 "\n",
                    req_id, (uint32_t)command_data_size)
      ...
      if (EmwOsInterface::eOK != EmwOsInterface::takeSemaphore(&EmwCoreIpc::PendingRequest.responseSem, timeoutInMs)) {
        DEBUG_IPC_LOG("EmwCoreIpc::request(): Error: command 0x%04" PRIx32 " timeout(%" PRIu32 " ms)" \
                      ...
                      (uint32_t)api_id, timeoutInMs, EmwCoreIpc::PendingRequest.reqId)
      ...
      DEBUG_IPC_LOG("EmwCoreIpc::request()< req_id: 0x%08" PRIx32 " api_id: 0x%04" PRIx32 " "
                    "done (%" PRId32 ")\n\n",
                    req_id, (uint32_t)api_id, (int32_t)ret)
    }
...
EmwCoreIpc::Status EmwCoreIpc::testIpcEcho(const class EmwApiCore &core,
    uint8_t *dataInPtr, uint16_t dataInLength, uint8_t *dataOutPtr, uint16_t *dataOutLengthPtr,
    uint32_t timeoutMs)
{
  EmwCoreIpc::Status ret = EmwCoreIpc::eERROR;

  if ((nullptr != dataInPtr) \
      && (EmwCoreIpc::PACKET_MIN_SIZE <= dataInLength) \
    ...
    if (EmwCoreIpc::eSUCCESS != EmwCoreIpc::request(core,
        &dataInPtr[PACKET_PARAMS_OFFSET], (uint16_t)(dataInLength - EmwCoreIpc::HEADER_SIZE),
        dataOutPtr, dataOutLengthPtr, timeoutMs)) {
      *dataOutLengthPtr = 0U;
    }
    else {
...
void EmwCoreIpc::processEvent(const class EmwApiCore &core, EmwNetworkStack::Buffer_t *networkBufferPtr)
{
  ...
  if ((nullptr != payload_ptr) && (EmwCoreIpc::PACKET_MIN_SIZE <= payload_size)) {
    ...
    DEBUG_IPC_LOG("EmwCoreIpc::event(): req_id: 0x%08" PRIx32 ", api_id: 0x%04" PRIx32 "\n",
                  req_id, (uint32_t)api_id)

    if ((0U == (api_id & MIPC_API_EVENT_BASE)) /*&& (MIPC_REQ_ID_NONE != req_id)*/) {
      if (EmwCoreIpc::PendingRequest.reqId == req_id) {
        if ((nullptr != EmwCoreIpc::PendingRequest.responseBufferSizePtr) \
          ...
          *EmwCoreIpc::PendingRequest.responseBufferSizePtr = (uint16_t)actual_size;
        }
        ...
        DEBUG_IPC_LOG("EmwCoreIpc::event()  : response req_id: 0x%08" PRIx32
      ...
      for (i = 0U; i < event_table_count; i++) {
        if (((uint16_t)event_table[i].eventId) == api_id) {
      ...
      if (i == event_table_count) {
        DEBUG_IPC_LOG("EmwCoreIpc::event()  : Unknown event: 0x%04" PRIx32 "!\n", (uint32_t)api_id)
    ...
    DEBUG_IPC_LOG("EmwCoreIpc::event()  : Unknown buffer content\n")
    EmwCoreHci::free(networkBufferPtr);
  }
}
...
void EmwCoreIpc::wiFiStatusEventCallback(const class EmwApiCore &core, EmwNetworkStack::Buffer_t *networkBufferPtr)
{
  ...
  const enum EmwApiBase::WiFiEvent event = (enum EmwApiBase::WiFiEvent)status;
...
void EmwCoreIpc::wiFiNetlinkInputCallback(const class EmwApiCore &core, EmwNetworkStack::Buffer_t *networkBufferPtr)
{
  ...
  if ((nullptr != core.runtime.netlinkInputCallback) && (0U < parameters_ptr->totalLength)) {
    uint32_t low_level_netif_idx = (uint32_t)parameters_ptr->idx;
    EmwNetworkStack::hideBufferHeader(networkBufferPtr);
    core.runtime.netlinkInputCallback(networkBufferPtr, static_cast<void *>(&low_level_netif_idx));
  }
  else {
...
{
  const uint16_t api_id \
    = (uint16_t)((uint16_t)buffer[PACKET_API_ID_OFFSET + 1U] << 8) \
      | (uint16_t)buffer[PACKET_API_ID_OFFSET];
  return api_id;
}
...
uint32_t EmwCoreIpc::getReqId(const uint8_t buffer[])
{
  const uint32_t req_id \
    = ((uint32_t)buffer[PACKET_REQ_ID_OFFSET + 3U] << 24) \
      | ((uint32_t)buffer[PACKET_REQ_ID_OFFSET + 2U] << 16) \
      | ((uint32_t)buffer[PACKET_REQ_ID_OFFSET + 1U] << 8) \
      | (uint32_t)buffer[PACKET_REQ_ID_OFFSET];
  return req_id;
}
...
uint8_t *EmwCoreIpc::startWithHeader(uint8_t *ipcBytesPtr)
{
  return static_cast<uint8_t *>(ipcBytesPtr - EmwCoreIpc::PACKET_PARAMS_OFFSET);
}
...
{
  buffer[PACKET_API_ID_OFFSET] = (uint8_t)(apiId & 0x00FFU);
  buffer[PACKET_API_ID_OFFSET + 1U] = (uint8_t)((apiId & 0xFF00U) >> 8);
}

void EmwCoreIpc::setReqId(uint8_t buffer[], uint32_t reqId)
{
  buffer[PACKET_REQ_ID_OFFSET] = (uint8_t)(reqId & 0x000000FFU);
  buffer[PACKET_REQ_ID_OFFSET + 1U] = (uint8_t)((reqId & 0x0000FF00U) >> 8);
  buffer[PACKET_REQ_ID_OFFSET + 2U] = (uint8_t)((reqId & 0x00FF0000U) >> 16);
  buffer[PACKET_REQ_ID_OFFSET + 3U] = (uint8_t)((reqId & 0xFF000000U) >> 24);
}
