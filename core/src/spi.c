...
void HAL_SPI_MspInit(SPI_HandleTypeDef *handlePtr)
{
  ...
  __HAL_LINKDMA(handlePtr, hdmatx, hGpdma1Channel5);
  ...
  __HAL_LINKDMA(handlePtr, hdmarx, hGpdma1Channel4);
...
void HAL_SPI_MspDeInit(SPI_HandleTypeDef *handlePtr)
{
  if (handlePtr->Instance == SPI2) {
    ...
    HAL_DMA_DeInit(handlePtr->hdmatx);
    HAL_DMA_DeInit(handlePtr->hdmarx);
