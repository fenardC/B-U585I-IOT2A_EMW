...
std::int32_t AppConsolePing::doPing(const char (&peerNameString)[255]) noexcept
{
  ...
  if (AppLwipService::getHostByName(reinterpret_cast<struct sockaddr *>(&sock_address_in),
                                    peerNameString) < 0) {
    ...
    if (this->icmpPing(reinterpret_cast<struct sockaddr *>(&sock_address_in), response_size,
                       AppConsolePing::PING_DELAY_MS,
                       reinterpret_cast<std::int32_t(&)[10]>(responses)) == 0) {
      status = 0;
...
std::int32_t AppConsolePing::doPing6(const char (&peerNameString)[255]) noexcept
{
  ...
  if (AppLwipService::getHostByName(reinterpret_cast<struct sockaddr *>(&sock_address_in6),
                                    peerNameString) < 0) {
    STD_PRINTF(" %s: Failed to find the host name \"%s\"\n", this->getName(), peerNameString);
  }
  else {
    ...
    if (this->icmpPing(reinterpret_cast<struct sockaddr *>(&sock_address_in6),
