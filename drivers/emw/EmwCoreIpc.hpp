...
#define ID_NONE (static_cast<uint16_t>(0x0000U))
#define CMD_BASE (static_cast<uint16_t>(ID_NONE))
  public:
    enum ApiId : std::uint16_t {
      eSYS_CMD_BASE = static_cast<uint16_t>(0U),
      eSYS_ECHO_CMD,
      eSYS_REBOOT_CMD,
      eSYS_VERSION_CMD,
      eSYS_RESET_CMD,
      ...
      eWIFI_CMD_BASE = static_cast<uint16_t>(CMD_BASE + 0x0100U),
      eWIFI_GET_MAC_CMD,
      eWIFI_SCAN_CMD,
      eWIFI_CONNECT_CMD,
      ...
      eWIFI_PING6_CMD,
      eSOCKET_CMD_BASE = static_cast<uint16_t>(CMD_BASE + 0x0200U),
      eSOCKET_CREATE_CMD,
      eSOCKET_CONNECT_CMD,
      eSOCKET_SEND_CMD,
      eSOCKET_SENDTO_CMD,
  ...
  public:
    ...
    typedef __PACKED_STRUCT SysCommonResponseParams_s {
      SysCommonResponseParams_s(void): status(-1) {}
      int32_t status;
...
#define MIPC_API_EVENT_BASE (static_cast<uint16_t>(0x8000U))
  private:
    enum EventId {
      eSYS_EVENT_BASE = static_cast<uint16_t>(MIPC_API_EVENT_BASE + 0x0000U),
      eSYS_REBOOT_EVENT,
      eSYS_FOTA_STATUS_EVENT,
      eWIFI_EVENT_BASE = static_cast<uint16_t>(MIPC_API_EVENT_BASE + 0x0100U),
      eWIFI_STATUS_EVENT,
