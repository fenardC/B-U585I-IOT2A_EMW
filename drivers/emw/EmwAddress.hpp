...
#define EMW_FD_ZERO(P) memset((P), 0, sizeof(*(P)))
...
class EmwAddress final {
  ...
  public:
    typedef struct IpAddr_s {
      constexpr IpAddr_s(void): addr(0U) {}
      constexpr explicit IpAddr_s(uint32_t address): addr(address) {}
      std::uint32_t addr;
    } IpAddr_t;

    typedef struct Ip6Addr_s {
      constexpr Ip6Addr_s(void): addr{0U}, zone(0) {}
      constexpr explicit Ip6Addr_s(uint32_t block0, uint32_t block1, uint32_t block2, uint32_t block3)
    ...
    typedef struct SockAddr_s {
      constexpr SockAddr_s(void): length(0U), family(0U), data{0U} {}
      uint8_t length;
      uint8_t family;
      uint8_t data[14];
    } SockAddr_t;
    ...
    typedef struct InAddr_s {
      constexpr InAddr_s(void): addr(0U) {}
      constexpr explicit InAddr_s(uint32_t address): addr(address) {}
      uint32_t addr;
    } InAddr_t;

    typedef struct SockAddrIn_s {
      constexpr SockAddrIn_s(void)
        : length(sizeof(*this)), family(EMW_AF_INET), port(0U), inAddr(), zero{0U} {}
      constexpr explicit SockAddrIn_s(uint16_t port, uint32_t address)
    ...
    typedef struct In6Addr_s {
      constexpr In6Addr_s(void)
        : un{0} {}
      constexpr In6Addr_s(uint32_t data0, uint32_t data1, uint32_t data2, uint32_t data3)
    ...
    typedef struct SockAddrIn6_s {
      constexpr SockAddrIn6_s(void)
        : length(sizeof(*this)), family(EMW_AF_INET6), port(0U), flowInfo(0U), in6Addr(), scopeId{0U} {}
      constexpr explicit SockAddrIn6_s(uint16_t port, const In6Addr_t &address)
    ...
    typedef __PACKED_STRUCT SockAddrStorage_s{
      constexpr SockAddrStorage_s(void): length(sizeof(*this)), family(EMW_AF_UNSPEC), data1{0U, 0U}, data2{0U, 0U, 0U}, data3{0U, 0U, 0U} {}
      uint8_t length;
      uint8_t family;
      uint8_t data1[2];
      uint32_t data2[3];
    ...
    typedef __PACKED_STRUCT AddrInfo_s {
      constexpr AddrInfo_s(void)
        : flags(0), family(0), sockType(0), protocol(0), addrLen(0U), sAddr(), canonName{0}, nextPtr(nullptr) {}
      constexpr explicit AddrInfo_s(int32_t flags, int32_t family, int32_t sockType, int32_t protocol)
  ...
  public:
    static int32_t asciiToNetwork(const char *addressStringPtr, EmwAddress::IpAddr_t &address);
  public:
    static int32_t asciiToNetwork(const char *addressStringPtr, EmwAddress::Ip6Addr_t &address);
  public:
    static char *networkToAscii(const EmwAddress::IpAddr_t &address, char buffer[], const std::size_t bufferSize);
  public:
    static char *networkToAscii(const EmwAddress::Ip6Addr_t &address, char buffer[], const std::size_t bufferSize);
  private:
    static uint32_t hostToNetworkLong(uint32_t hostLong);
  private:
    static bool isDigit(uint8_t byte);
  private:
    static bool isHexaDigit(uint8_t byte);
  private:
    static bool isLower(uint8_t byte);
  private:
    static bool isSpace(uint8_t byte);
  private:
    static char toDecimalChar(uint8_t halfByte);
};
