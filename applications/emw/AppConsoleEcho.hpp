...
class AppConsoleEcho final : public Cmd {
  ...
  private:
    int32_t doEcho(const EmwAddress::SockAddrIn_t &sAddressIn, uint32_t loop, uint32_t untilN);
  private:
    int32_t doEcho6(const EmwAddress::SockAddrIn6_t &sAddressIn6, uint32_t loop, uint32_t untilN);
  private:
