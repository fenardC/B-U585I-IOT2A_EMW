...
EmwNetworkStack::Buffer_t *EmwCoreHci::receive(uint32_t timeoutInMs)
{
  ...
  const EmwOsInterface::Status os_status = EmwOsInterface::getMessageQueue(&EmwCoreHci::NetworkPacketFifo,
    timeoutInMs, &message_ptr);
...
#if 0
    {
      const uint32_t len = EmwNetworkStack::getBufferPayloadSize(network_buffer_ptr);
      DEBUG_HCI_LOG("\nEmwCoreHci::receive(): %p %" PRIu32 "\n", (void *)network_buffer_ptr, len)
    }
#endif /* 0 */
  }
...
{
  int32_t ret = 0;
#if defined(EMW_USE_SPI) || defined(EMW_USE_SPI_DMA)
  {
