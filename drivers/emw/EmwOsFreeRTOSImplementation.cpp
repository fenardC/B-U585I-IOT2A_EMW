    ...
    const char *semaphoreNamePtr, std::uint32_t maxCount, std::uint32_t initialCount) noexcept
{
  ...
  EMW_OS_DEBUG_LOG("\n EmwOsInterface::CreateSemaphore()> \"%s\"\n",
                   (semaphoreNamePtr != nullptr) ? semaphoreNamePtr : "")
    ...
    std::uint32_t timeoutInMs) noexcept
{
  ...
  if (xQueueSemaphoreTake(semaphore, MilliSecondsToTicks(timeoutInMs)) == PASS) {
    status = EmwOsInterface::eOK;
  }
  else {
...
EmwOsInterface::Status EmwOsInterface::ReleaseSemaphore(EmwOsInterface::Semaphore_t &semaphore) noexcept
{
  ...
  if (xPortIsInsideInterrupt() == TRUE) {
    BaseType_t yield = FALSE;

    if (xQueueGiveFromISR(semaphore, &yield) == PASS) {
      if (yield == TRUE) {
        taskYIELD();
      }
      status = EmwOsInterface::eOK;
    ...
    if (xQueueGenericSend(semaphore, nullptr, GIVE_BLOCK_TIME, SEND_TO_BACK) == PASS) {
      status = EmwOsInterface::eOK;
    }
  }
  return status;
...
EmwOsInterface::Status EmwOsInterface::TakeMutex(EmwOsInterface::Mutex_t &mutex, std::uint32_t timeoutInMs) noexcept
{
  ...
  if (xQueueTakeMutexRecursive(mutex, MilliSecondsToTicks(timeoutInMs)) == PASS) {
    status = EmwOsInterface::eOK;
  }
  else {
    status = EmwOsInterface::eERROR;
...
EmwOsInterface::Status EmwOsInterface::ReleaseMutex(EmwOsInterface::Mutex_t &mutex) noexcept
{
  ...
  if (xQueueGiveMutexRecursive(mutex) == PASS) {
    status = EmwOsInterface::eOK;
  }
  else {
    status = EmwOsInterface::eERROR;
    ...
    std::uint32_t timeoutInMs) noexcept
{
  ...
  if (messagePtr != nullptr) {
    ...
    if (xQueueGenericSend(queue, &item, MilliSecondsToTicks(timeoutInMs), SEND_TO_BACK) == PASS) {
      status = EmwOsInterface::eOK;
    }
  }
  return status;
    ...
    const void *&messagePtr) noexcept
{
  ...
  messagePtr = nullptr;
  if (xQueueReceive(queue, &message_ptr, MilliSecondsToTicks(timeoutInMs)) == PASS) {
    ...
    std::uint32_t stackSize, EmwOsInterface::ThreadPriority_t priority) noexcept
{
  ...
  if ((stackSize >= EMW_OS_MINIMAL_THREAD_STACK_SIZE)) {
    ...
    if (xTaskCreate(reinterpret_cast<TaskFunction_t>(function), (nullptr != threadNamePtr) ? threadNamePtr : "",
                    stack, const_cast<void *>(argument), static_cast<UBaseType_t>(priority),
                    &the_thread) == PASS) {
      if (the_thread != nullptr) {
        thread = the_thread;
        status = EmwOsInterface::eOK;
      }
    }
