...
static void CheckEmwIoSpeeds(EmwApiCore &emw, std::uint16_t data_size, std::uint32_t loop_count) noexcept;
...
void InitializeEmw(EmwApiCore &emw) noexcept
{
  ...
  CheckEmwIoSpeeds(emw, 1542U, 130U);
  CheckEmwIoSpeeds(emw, 2500U, 80U);
}

static void CheckEmwIoSpeeds(EmwApiCore &emw, std::uint16_t data_size, std::uint32_t loop_count) noexcept
{
#if defined(COMPILATION_WITH_NO_OS)
  std::unique_ptr<std::uint8_t[]> ipc_test_data_ptr(new (std::nothrow) std::uint8_t[data_size]);
#elif defined(COMPILATION_WITH_FREERTOS)
  std::unique_ptr<std::uint8_t[], decltype(&vPortFree)> \
  ipc_test_data_ptr(static_cast<std::uint8_t*>(pvPortMalloc(data_size)), &vPortFree);
#endif /* COMPILATION_WITH_NO_OS */

  if (ipc_test_data_ptr != nullptr) {
    std::uint32_t send_count = 0U;
    std::uint32_t receive_count = 0U;
    std::uint32_t time_ms_count;

    std::memset(&ipc_test_data_ptr[0], 0x00, data_size);

    STD_PRINTF("\n[%6" PRIu32 "] Checking Emw Io Speed (%" PRIu32 " x (%" PRIu32 " + %" PRIu32 ")) ...\n",
               HAL_GetTick(), loop_count,
               static_cast<std::uint32_t>(data_size), static_cast<std::uint32_t>(data_size));

    time_ms_count = HAL_GetTick();

    for (std::uint32_t i = 0U; i < loop_count; i++) {
      std::uint16_t echoed_length = data_size;
      const EmwApiBase::Status status \
        = emw.testIpcEcho(reinterpret_cast<std::uint8_t (&)[]>(* &ipc_test_data_ptr[0]), data_size,
...
std::int32_t Scan(EmwApiCore &emw) noexcept
{
  ...
  if (emw.scan(EmwApiBase::ePASSIVE, ssid, 0) == EmwApiBase::eEMW_STATUS_OK) {
    status = 0;
