...
class AppDhcpService final {
  ...
  private:
    int32_t handleDiscoverMessage(bool isAddressSeenBefore, std::uint32_t offerAddress,
                                  const std::uint8_t *serviceReqPtr, std::uint32_t serviceReqLength,
  ...
  private:
    static int32_t FindIndex(const AppDhcpService::DhcpAddress_t addressInfosStorage[], std::uint32_t infosCount,
  ...
  private:
    bool storeInfo(const AppDhcpService::DhcpAddress_t &addressInfo) noexcept;
  ...
  private:
    const std::uint32_t MINIMUM_IP_ADDRESS;
  private:
    const std::uint32_t MAXIMUM_IP_ADDRESS;
  private:
