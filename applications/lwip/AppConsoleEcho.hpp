...
class AppConsoleEcho final : public Cmd {
  ...
  private:
    std::int32_t doEcho(const struct sockaddr_in &sockAddressIn, std::uint32_t loop, std::uint32_t untilN) noexcept;
  private:
    std::int32_t doEcho6(const struct sockaddr_in6 &sockAddressIn6, std::uint32_t loop,
