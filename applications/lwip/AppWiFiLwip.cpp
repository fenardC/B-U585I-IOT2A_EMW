  ...
  , emwNetworkStation(lwipNetifStation)
{
  ...
  this->lwipNetifSoftAp.state = &this->emwNetworkSoftAp;

  this->lwipNetifStation.name[0] = 'M';
  this->lwipNetifStation.name[1] = 'S';
  this->lwipNetifStation.state = &this->emwNetworkStation;
...
void AppWiFiLwip::connectToAp(const char (&ssidString)[33], const char (&passwordString)[65]) noexcept
{
  ...
  if (this->emwNetworkStation.activate(ssidString, passwordString) != 0) {
...
void AppWiFiLwip::constructSoftApNetworkInterface(void) noexcept
{
  ...
  if (netif_add_noaddr(&this->lwipNetifSoftAp, &this->emwNetworkSoftAp,
                       &WiFiNetworkSoftAp::InitializeNetif, &tcpip_input) == nullptr) {
    ErrorHandler();
    return;
  }
...
void AppWiFiLwip::constructStationNetworkInterface(void) noexcept
{
  ...
  if (netif_add_noaddr(&this->lwipNetifStation, &this->emwNetworkStation,
                       &WiFiNetworkStation::InitializeNetif, &tcpip_input) == nullptr) {
    ErrorHandler();
    return;
  }
...
void AppWiFiLwip::enableSoftAp(const char (&ssidString)[33], const char (&passwordString)[65]) noexcept
{
  ...
  if (this->emwNetworkSoftAp.activate(ssidString, passwordString) != 0) {
  ...
  STD_PRINTF("Starting the DHCP server ...\n");
  {
    ...
    if (dhcp_server.createService() != 0) {
