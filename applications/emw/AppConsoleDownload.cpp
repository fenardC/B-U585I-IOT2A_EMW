...
std::int32_t AppConsoleDownload::doDownload(const char *hostPtr, const char *requestPtr) noexcept
{
  ...
  if (Emw.socketGetHostByName(reinterpret_cast<EmwAddress::SockAddr_t &>(sock_address_in),
                              reinterpret_cast<const char(&)[255]>(*hostPtr)) < 0) {
    ...
    try {
      this->socket = Emw.socketCreate(EMW_AF_INET, EMW_SOCK_STREAM, EMW_IPPROTO_TCP);
      if (this->socket < 0) {
      ...
      }
      {
        ...
        std::int32_t socket_status = Emw.socketSetSockOpt(this->socket,
        ...
        socket_status = Emw.socketSetSockOpt(this->socket,
      ...
      {
        const std::int32_t connect_status = Emw.socketConnect(this->socket,
      ...
      status = -1;
      if (this->socket >= 0) {
        static_cast<void>(Emw.socketClose(this->socket));
      }
    }
  }
  DEBUG_STD_PRINTF("\n AppConsoleDownload::doDownload()<\n")
...
std::int32_t AppConsoleDownload::doDownload6(const char *hostPtr, const char *requestPtr) noexcept
{
  ...
  if (Emw.socketGetAddrInfo(reinterpret_cast<const char(&)[255]>(*hostPtr),
                            reinterpret_cast<const char (&)[255]>(empty), hints, result) < 0) {
    STD_PRINTF(" %s: failed to find the host name \"%s\"\n", this->getName(), hostPtr);
  }
  else {
    ...
    try {
      ...
      this->socket = Emw.socketCreate(EMW_AF_INET6, EMW_SOCK_STREAM, EMW_IPPROTO_TCP);
      if (this->socket < 0) {
        throw std::runtime_error("socketCreate() failed");
      }
  ...
  try {
    ...
    *(buffer_ptr.get() + buffer_size - 1) = '\0';
    ...
    while (length) {
      ...
      const std::int32_t count = Emw.socketReceive(this->socket,
    ...
    static_cast<void>(Emw.socketShutDown(this->socket, 2));
    static_cast<void>(Emw.socketClose(this->socket));
    {
    ...
    std::uint32_t &retSize) const noexcept
{
  ...
  while (header_size > 0) {
    const std::int32_t ret = Emw.socketReceive(this->socket,
