...
int main(void)
{
  ...
  InitializeRNG();
  {
    ...
    uint16_t welcome_size = static_cast<uint16_t>(std::strlen(welcome));
  ...
  std::setbuf(stdin, NULL);
  {
...
#if defined(__GNUC__)
    {
      ...
      std::strncat(the_compiler_string, gnu_compiler_string, the_compiler_string_size - strlen(the_compiler_string));
    }
#endif /* __GNUC__ */
    {
  ...
  }
  {
    ...
    std::printf("std::rand() seeded by %" PRId32 " returned %" PRId32 "\n\n",
                static_cast<int32_t>(random_number), static_cast<int32_t>(std::rand()));
  }
  CheckArmCore();
...
#if defined(COMPILATION_WITH_FREERTOS)
  std::printf("main(): StackType_t with: %" PRIu32 "\n", static_cast<uint32_t>(sizeof(StackType_t)));
...
static void CheckArmCore(void)
{
  const uint32_t cpuid = (reinterpret_cast<SCB_Type *>(SCB_BASE))->CPUID;
  const uint32_t idcode = (reinterpret_cast<DBGMCU_TypeDef *>(DBGMCU_BASE))->IDCODE;
  uint32_t var;
  uint32_t pat;
