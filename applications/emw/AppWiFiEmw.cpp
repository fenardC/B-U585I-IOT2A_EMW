...
{
  uint8_t *const ipc_test_data_ptr = new uint8_t[EmwNetworkStack::NETWORK_BUFFER_SIZE];
...
void AppWiFiEmw::connectAp(const char *ssidStringPtr, const char *passwordStringPtr)
{
  ...
  if (EmwApiBase::eEMW_STATUS_OK != status) {
    std::printf("[%6" PRIu32 "] Cannot Join ... \"%s\" (%" PRId32 ")\n", HAL_GetTick(), ssidStringPtr, (int32_t)status);
    ErrorHandler();
    return;
  }
  {
  ...
  if (1 == this->emw.isConnected()) {
    {
      ...
      std::printf("\nIP address      %02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 "\n",
                  (uint32_t)ip_address_bytes[0], (uint32_t)ip_address_bytes[1],
                  (uint32_t)ip_address_bytes[2], (uint32_t)ip_address_bytes[3]);
      std::printf("Gateway address %02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 "\n",
                  (uint32_t)this->emw.stationSettings.gatewayAddress[0],
                  (uint32_t)this->emw.stationSettings.gatewayAddress[1],
                  (uint32_t)this->emw.stationSettings.gatewayAddress[2],
                  (uint32_t)this->emw.stationSettings.gatewayAddress[3]);
      std::printf("DNS1 address    %02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 "\n",
                  (uint32_t)this->emw.stationSettings.dns1[0], (uint32_t)this->emw.stationSettings.dns1[1],
                  (uint32_t)this->emw.stationSettings.dns1[2], (uint32_t)this->emw.stationSettings.dns1[3]);
    }
    for (uint8_t interface = 0U; interface < EmwApiBase::eWIFI_INTERFACE_IDX_MAX; interface++) {
      for (uint8_t address_slot = 0U; address_slot < 3U; address_slot++) {
        uint8_t ip_addr[16] = {0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U};
        const int32_t address_state = this->emw.getIP6AddressState(address_slot, (EmwApiBase::EmwInterface)interface);

        std::printf("IPv6 address (%" PRIu32 ")(%" PRIu32 "): state %" PRIi32 "\n",
                    (uint32_t)interface, (uint32_t)address_slot, address_state);
        status = this->emw.getIP6Address(ip_addr, address_slot, (EmwApiBase::EmwInterface)interface);
        if (EmwApiBase::eEMW_STATUS_OK != status) {
          std::printf("failed to get IPv6 address (%" PRId32 ")\n", (int32_t)status);
        }
        std::printf("IPv6 address (%" PRIu32 ")(%" PRIu32 "): "
                    "%02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n\n",
                    (uint32_t)interface, (uint32_t)address_slot,
  ...
  std::printf("AppWiFiEmw::connectAp()<\n\n");
}

void AppWiFiEmw::disableSoftAp(void)
{
  STD_PRINTF("\nAppWiFiEmw::disableAP()>\n")
...
void AppWiFiEmw::enableSoftAp(void)
{
...

  {
    ...
    if (EmwApiBase::eEMW_STATUS_OK != status) {
      std::printf("Failed to setup %s (%" PRId32 ")\n", access_point_settings.ssidString, (int32_t)status);
      ErrorHandler();
      return;
    }
