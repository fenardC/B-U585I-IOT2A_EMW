...
void WiFiNetworkCore::InitializeSystem(void) noexcept
{
  DEBUG_STD_PRINTF("WiFiNetworkCore::InitializeSystem()>\n")

  if (sys_mbox_new(&WiFiNetworkCore::TransmitFifo, 8U) == static_cast<err_t>(ERR_OK)) {
  ...
  static_cast<void>(EmwBypass.setByPass(1, &WiFiNetworkCore::InputFromEmw));
...
void WiFiNetworkCore::UnInitializeSystem(void) noexcept
{
  ...
  if (EmwBypass.numberOfInterfacesRunning() == 0U) {
...
std::int32_t WiFiNetworkCore::UnInitializeEmw(void) noexcept
{
  ...
  if (EmwBypass.numberOfInterfacesRunning() == 1U) {
    static_cast<void>(EmwBypass.setByPass(0, nullptr));
  }
  EmwBypass.unInitialize();
  DEBUG_STD_PRINTF("WiFiNetworkCore::unInitializeEmw()<\n")
  return 0;
}
...
void WiFiNetworkCore::InputFromEmw(EmwNetworkStack::Buffer_t *bufferPtr, std::uint32_t interfaceIndex) noexcept
{
  if (bufferPtr != nullptr) {
    if (EmwNetworkStack::GetBufferPayloadSize(bufferPtr) > 0U) {

      if (interfaceIndex == EmwApiBase::eWIFI_INTERFACE_STATION_IDX) {
        WiFiNetworkStationPtr->inputNetif(bufferPtr);
      }
      else if (interfaceIndex == EmwApiBase::eWIFI_INTERFACE_SOFTAP_IDX) {
...
void WiFiNetworkCore::PushToEmw(std::uint32_t timeoutInMs) noexcept
{
  ...
  if (sys_arch_mbox_fetch(&WiFiNetworkCore::TransmitFifo, &message_ptr,
                          timeoutInMs) != SYS_ARCH_TIMEOUT) {
    if (message_ptr != nullptr) {
      struct pbuf *const packet_ptr = static_cast<struct pbuf *>(message_ptr);
      const EmwApiBase::Status \
      ret = EmwBypass.output(static_cast<std::uint8_t*>(packet_ptr->payload),
                             static_cast<std::int32_t>(packet_ptr->len),
                             static_cast<std::int32_t>(packet_ptr->if_idx));

      LWIP_ASSERT("PushToEmw() failed", ret == EmwApiBase::eEMW_STATUS_OK);
