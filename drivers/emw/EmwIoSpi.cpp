...
void EmwIoSpi::initializeImp(EmwIoInterfaceTypes::InitializationMode mode) noexcept
{
  ...
  if (mode == EmwIoInterfaceTypes::eRESET) {
...
void EmwIoSpi::processPollingDataImp(std::uint32_t timeoutInMs) noexcept
{
  ...
  while (network_buffer_ptr == nullptr) {
  ...
  if (EmwOsInterface::TakeSemaphore(TxRxSem, timeoutInMs) == EmwOsInterface::eOK) {
...
std::uint16_t EmwIoSpi::sendImp(const std::uint8_t *dataPtr, std::uint16_t dataLength) noexcept
{
  ...
  if (dataLength > EmwIoSpi::SPI_MAX_BYTE_COUNT) {
    ...
    if (EmwOsInterface::ReleaseSemaphore(TxRxSem) != EmwOsInterface::eOK) {
...
std::int8_t EmwIoSpi::waitFlowHigh(void) noexcept
{
  ...
  if (EmwOsInterface::TakeSemaphore(FowRiseSem, TIMEOUT_HARDWARE_EMW_MS) != EmwOsInterface::eOK) {
    status = -1;
  }
  if (this->isFlowLow()) {
...
std::int32_t EmwIoSpi::exchangeHeaders(std::uint16_t txLength, std::uint16_t &rxLength) noexcept
{
  ...
  if (this->transmitReceive(reinterpret_cast<const std::uint8_t *>(&spi_master_header),
                            reinterpret_cast<std::uint8_t *>(&spi_slave_header),
                            sizeof(spi_master_header)) != HAL_OK) {
    DRIVER_ERROR_VERBOSE("Send SPI master header error\n")
  }
  else if (spi_slave_header.type != SPI_READ) {
  ...
  else if ((spi_master_header.len == 0U) && (spi_slave_header.len == 0U)) {
