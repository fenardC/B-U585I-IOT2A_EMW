...
EmwNetworkStack::Buffer_t *EmwNetworkStack::allocBuffer(void)
{
  ...
  if (nullptr != network_packet_ptr) {
    DEBUG_IO_LOG("\nEmwNetworkStack::allocBuffer(): allocated %p (%" PRIu32 ")\n",
                 static_cast<void *>(network_packet_ptr), static_cast<uint32_t>(EmwNetworkStack::NETWORK_BUFFER_SIZE))
...
{
  pbuf_realloc(EmwBufferToLwipBuffer(networkPacketPtr), static_cast<u16_t>(size));
}
...
const uint16_t EmwNetworkStack::NETWORK_MTU_SIZE = EMW_NETWORK_MTU_SIZE_DEFINED;
const uint16_t EmwNetworkStack::NETWORK_BUFFER_SIZE = NETWORK_BUFFER_SIZE_DEFINED;
const uint16_t EmwNetworkStack::NETWORK_IPC_PAYLOAD_SIZE = NETWORK_BUFFER_SIZE_DEFINED - 6U;
