...
void AppHttpSSE::initializeServer(uint32_t ipAddressIn, uint16_t port)
{
  const char webserver_task_name[] = {"HTTP-Main"};

  AppHttpSSE::serverIpAddrIn = ipAddressIn;
  AppHttpSSE::serverPort = port;
  xTaskCreate(AppHttpSSE::doListenService, webserver_task_name,
              AppHttpSSE::WEBSERVER_TASK_STACK_SIZE, this,
              AppHttpSSE::WEBSERVER_TASK_PRIORITY, NULL);
}

uint32_t AppHttpSSE::convertToOsTicks(uint32_t milliseconds)
{
  return static_cast<uint32_t>((static_cast<uint64_t>(milliseconds) * static_cast<uint64_t>(configTICK_RATE_HZ)) / 1000U);
}

void AppHttpSSE::doAcceptService(void *argPtr)
{
  const int32_t sock = *(static_cast<int32_t *>(argPtr));
  ...
  try {
    ...
    peer.infos();
    {
      const int32_t count = lwip_recv(sock, receive_buffer_ptr.get(), receive_buffer_length, 0);
      STD_PRINTF("\n%" PRIi32 " >>> %" PRIi32 " <<<\n", sock, count)
      if (count > 0) {
...
void AppHttpSSE::doListenService(void)
{
  int32_t sock;
  ...
  try {
    ...
    }
    {
      ...
      const struct ifreq iface = {{this->netif.name[0], netif.name[1], static_cast<char>('0' + netif.num), '\0', 0, 0}};
    ...
    while (true) {
      ...
      uint32_t s_remote_in_size = sizeof(s_remote_in);

      int32_t connected_sock = lwip_accept(sock, reinterpret_cast<struct sockaddr *>(&s_remote_in), &s_remote_in_size);
      if (0 > connected_sock) {
...

      {
        ...
        ret = xTaskCreate(doAcceptService, task_name,
...
void AppHttpSSE::listTasks(char *bufferStringPtr, size_t bufferStringSize)
{
  std::strcat(bufferStringPtr, \
              "Name                    State  Prio  Stack  Num  \n" \
              "------------------------------------------------ \n");
  vTaskListTasks(bufferStringPtr + strlen(bufferStringPtr), bufferStringSize - (2 * strlen(bufferStringPtr)));
  std::strcat(bufferStringPtr, \
              "------------------------------------------------ \n" \
              "X:Running B:Blocked R:Ready D:Deleted S:Suspended\n");
  STD_PRINTF("%" PRIu32 "\n", static_cast<uint32_t>(strlen(bufferStringPtr)))
}

void AppHttpSSE::listRemainingFreeHeap(char *bufferStringPtr, size_t bufferStringSize)
{
  char free_heap[26];
  const size_t free_heap_size = sizeof(free_heap) - 1;
...
char *AppHttpSSE::chop(char *stringPtr)
{
...
int32_t AppHttpSSE::encodeHttpEvent(char *outBufferPtr, size_t outBufferLength, const char *eventNamePtr,
                                    char *messagePtr)
{
  static unsigned long id_counter = 0U;

  char *char_write_ptr = outBufferPtr;
  uint32_t char_write_left_count = static_cast<uint32_t>(outBufferLength - 1);
  uint32_t written;
  uint32_t chunked_size = 0U;
  char *chunked_size_ptr = nullptr;
...

  {
    ...
    while (nullptr != line_ptr) {
      ...
      line_ptr = AppHttpSSE::tokenize(tokenize_worker_ptr, '\n', tokenize_worker_ptr);
    }
...
char *AppHttpSSE::tokenize(char *stringPtr, char delimiter, char *&context)
{
  char *token_begin = nullptr;
  if (nullptr != stringPtr) {
    const char delimiter_string[] = {delimiter, '\0'};
    context = stringPtr;
    context += std::strspn(context, delimiter_string);
    if ('\0' == *context) {
      return nullptr;
    }
    token_begin = context;
    context += std::strcspn(context, delimiter_string);
    if ('\0' != *context) {
      *context++ = '\0';
    }
  }
...
AppHttpSSE::Client::ClientStatus AppHttpSSE::Client::getStatus(void)
{
  return this->status;
}

int32_t AppHttpSSE::Client::sendEvent(const char *eventNamePtr, char *bufferStringPtr)
{
  if (strlen(bufferStringPtr) == 0) {
    return 1;
  }
...
int32_t AppHttpSSE::Client::send(const char *bufferPtr, size_t bufferSize)
{
...
void AppHttpSSE::Client::setInfo(uint32_t address, uint16_t port)
{
  uint8_t peer_addr[4] = {0U, 0U, 0U, 0U};
  uint16_t peer_port = 0U;

  peer_addr[0] = static_cast<uint8_t>(address);
  peer_addr[1] = static_cast<uint8_t>(address >> 8);
  peer_addr[2] = static_cast<uint8_t>(address >> 16);
  peer_addr[3] = static_cast<uint8_t>(address >> 24);
  peer_port = ntohs(port);

  snprintf(this->info, sizeof(this->info) - 1,
           "%02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 ".%02" PRIu32 ":%5" PRIu32 "",
           static_cast<uint32_t>(peer_addr[0]), static_cast<uint32_t>(peer_addr[1]),
           static_cast<uint32_t>(peer_addr[2]), static_cast<uint32_t>(peer_addr[3]),
           static_cast<uint32_t>(peer_port));
  this->info[sizeof(this->info) - 1] = '\0';
}

void AppHttpSSE::Client::infos() const
{
...
int32_t AppHttpSSE::Client::close(void)
{
...
void AppHttpSSE::Client::sendIndexHtml(void)
{
...
void AppHttpSSE::Client::sendEventStream(void)
{
...
void AppHttpSSE::Client::sendWiFiIconPng(void)
{
...
void AppHttpSSE::Client::sendThreadsIconPng(void)
{
...
void AppHttpSSE::Client::sendMemoryIconPng(void)
{
...
void AppHttpSSE::Client::sendError404Html(void)
{
...
uint32_t AppHttpSSE::serverIpAddrIn = 0U;
uint16_t AppHttpSSE::serverPort = 0U;
